...
DUNGEON_WALL_H = 3.4
DUNGEON_WALL_T = 0.3
DUNGEON_DOOR_H = 2.3                   # doorway openings stop well short of
                                        # the ceiling, closed off above by a
                                        # lintel, so they read as real doors
DUNGEON_WALL_COLOR = (0.32, 0.29, 0.27, 1)
DUNGEON_FLOOR_COLOR = (0.30, 0.26, 0.21, 1)
DUNGEON_CEIL_COLOR = (0.22, 0.2, 0.19, 1)


def _build_doorway(parent, axis, fixed_coord, gap_center, gap_width):
    """A lintel closing the gap's top up to the ceiling, 2 frame posts at
    the opening's edges, and a single door leaf resting open to one side -
    all flat, axis-aligned, non-rotated boxes (matching make_box's
    centered-on-X/Y convention) so there's no hinge/rotation math to get
    wrong. Purely visual - the opening itself stays fully walkable."""
    door_color = (0.28, 0.18, 0.09, 1)
    frame_color = (0.18, 0.13, 0.08, 1)
    half_gap = gap_width / 2
    leaf_w = gap_width * 0.42
    thick = 0.06

    if axis == "x":
        if DUNGEON_WALL_H > DUNGEON_DOOR_H:
            lintel = make_box(gap_width, DUNGEON_WALL_T, DUNGEON_WALL_H - DUNGEON_DOOR_H,
                               DUNGEON_WALL_COLOR, "door_lintel")
            lintel.reparentTo(parent)
            lintel.setPos(gap_center, fixed_coord, DUNGEON_DOOR_H)
        for side in (-1, 1):
            post = make_box(0.07, DUNGEON_WALL_T + 0.05, DUNGEON_DOOR_H, frame_color, "door_post")
            post.reparentTo(parent)
            post.setPos(gap_center + side * (half_gap - 0.035), fixed_coord, 0)
        leaf = make_box(leaf_w, thick, DUNGEON_DOOR_H - 0.08, door_color, "door_leaf")
        leaf.reparentTo(parent)
        leaf.setPos(gap_center - half_gap + leaf_w / 2 + 0.07, fixed_coord, 0.02)
    else:
        if DUNGEON_WALL_H > DUNGEON_DOOR_H:
            lintel = make_box(DUNGEON_WALL_T, gap_width, DUNGEON_WALL_H - DUNGEON_DOOR_H,
                               DUNGEON_WALL_COLOR, "door_lintel")
            lintel.reparentTo(parent)
            lintel.setPos(fixed_coord, gap_center, DUNGEON_DOOR_H)
        for side in (-1, 1):
            post = make_box(DUNGEON_WALL_T + 0.05, 0.07, DUNGEON_DOOR_H, frame_color, "door_post")
            post.reparentTo(parent)
            post.setPos(fixed_coord, gap_center + side * (half_gap - 0.035), 0)
        leaf = make_box(thick, leaf_w, DUNGEON_DOOR_H - 0.08, door_color, "door_leaf")
        leaf.reparentTo(parent)
        leaf.setPos(fixed_coord, gap_center - half_gap + leaf_w / 2 + 0.07, 0.02)
...
def _dungeon_wall_run(parent, axis, fixed_coord, center_along, half_extent,
    ...
    make_box is centered on X/Y, so each segment's visual box and its
    collision polygon both just use their own midpoint directly."""
    if gap_width > 0:
    ...
    if gap_width > 0:
        _build_doorway(parent, axis, fixed_coord, gap_center, gap_width)


def build_dungeon_room(parent, cx, cy, doors):
    """doors: dict with keys 'n','s','e','w' -> bool, whether a doorway
    connects to a neighboring room on that side. Fully enclosed: floor,
    4 walls (each with a door where a neighbor exists), and a ceiling."""
    ...
    floor.setPos(cx, cy, -0.1)

    ceiling = make_box(DUNGEON_ROOM_W, DUNGEON_ROOM_D, 0.15, DUNGEON_CEIL_COLOR, "dceiling")
    ceiling.reparentTo(parent)
    ceiling.setPos(cx, cy, DUNGEON_WALL_H)
...
def _build_corridor(parent, axis, a0, a1, cross_center):
    """A short straight corridor (floor + ceiling + 2 side walls) filling
    the gap between two adjacent rooms' doorways. axis='x' runs the
    corridor along X (connecting east/west neighbors); axis='y' runs it
    along Y (connecting north/south neighbors)."""
    length = a1 - a0
    mid = (a0 + a1) / 2
    if axis == "x":
        floor = make_box(length, DUNGEON_CORR_W, 0.1, DUNGEON_FLOOR_COLOR, "corr_floor")
        floor.reparentTo(parent)
        floor.setPos(mid, cross_center, -0.1)
        ceiling = make_box(length, DUNGEON_CORR_W, 0.15, DUNGEON_CEIL_COLOR, "corr_ceiling")
        ceiling.reparentTo(parent)
        ceiling.setPos(mid, cross_center, DUNGEON_WALL_H)
        for side in (-1, 1):
        ...
        floor.reparentTo(parent)
        floor.setPos(cross_center, mid, -0.1)
        ceiling = make_box(DUNGEON_CORR_W, length, 0.15, DUNGEON_CEIL_COLOR, "corr_ceiling")
        ceiling.reparentTo(parent)
        ceiling.setPos(cross_center, mid, DUNGEON_WALL_H)
        for side in (-1, 1):
...
def build_dungeon_grid(parent, base_x, base_y):
    ...
    not by walking. Every room is fully enclosed (floor/walls/ceiling);
    horizontally/vertically adjacent rooms are joined by short covered
    corridors, with a real door (frame + open leaf) at every threshold.
    Enemies are spread across several rooms, and a return gate sits in the
    far corner room."""
    cols, rows = 5, 2
    ...
    void_ground.setPos(base_x + (cols - 1) * step_x / 2,
                        base_y + (rows - 1) * step_y / 2, -0.15)
