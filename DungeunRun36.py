    ...
    def _get_interior_layout(self, building):
        ...
        if btype == 'house':
            ...
            objs+=[{'type':'bed','x':ow+40,'y':ow+50,'w':bw,'h':bh,'collision':col(ow+40,ow+50,bw,bh)},
                   {'type':'wardrobe','x':ow+280,'y':ow+40,'w':80,'h':H//2-ow-60,'collision':col(ow+280,ow+40,80,H//2-ow-60)},
                   {'type':'nightstand','x':ow+250,'y':ow+70,'w':55,'h':55},
                   {'type':'candle','x':ow+277,'y':ow+65},
            ...
            ky=midY+iw
            objs+=[{'type':'stone_stove','x':ow+30,'y':ky+35,'w':110,'h':100,'collision':col(ow+30,ky+35,110,100)},
                   {'type':'pot','x':ow+85,'y':ky+22},
                   {'type':'kitchen_counter','x':ow+155,'y':ky+35,'w':180,'h':65,'collision':col(ow+155,ky+35,180,65)},
        ...
        # ── LIBRARY ────────────────────────────────────────────────────────
            ...
            SW = 100  # shelf unit depth — thick, prominent
            # Full-height bookcase LEFT
            objs.append({'type':'bookcase_unit','x':ow,'y':ow,'w':SW,'h':H-ow*2,'side':'left'})
            objs.append(conly(ow,ow,SW,H-ow*2))
            # Full-height bookcase RIGHT
            objs.append({'type':'bookcase_unit','x':W-ow-SW,'y':ow,'w':SW,'h':H-ow*2,'side':'right'})
            objs.append(conly(W-ow-SW,ow,SW,H-ow*2))
            # Shorter bookcase along TOP wall (leaves sides for tall ones)
            objs.append({'type':'bookcase_unit','x':ow+SW,'y':ow,'w':W-ow*2-SW*2,'h':SW,'side':'top'})
            objs.append(conly(ow+SW,ow,W-ow*2-SW*2,SW))

            # Reading desks — 4 total, upper half, flanking the circle
            for dx,dy in [(ow+SW+30,  ow+SW+30),
                          (W-ow-SW-150,ow+SW+30),
                          (ow+SW+30,  H*2//5),
                          (W-ow-SW-150,H*2//5)]:
                if safe(dx,dy,120,60):
                    objs+=[{'type':'reading_desk','x':dx,'y':dy,'w':120,'h':60,'collision':col(dx,dy,120,60)},
                           {'type':'candle','x':dx+100,'y':dy-8},
                           {'type':'open_book','x':dx+10,'y':dy+8,'w':70,'h':44}]

            # Row of desks near bottom (safe zone respected)
            for dx in [ow+SW+30, W//2-60, W-ow-SW-150]:
                dy2 = H-ow-250
                if safe(dx,dy2,120,60):
                    objs+=[{'type':'reading_desk','x':dx,'y':dy2,'w':120,'h':60,'collision':col(dx,dy2,120,60)},
                           {'type':'candle','x':dx+100,'y':dy2-8},
                           {'type':'open_book','x':dx+10,'y':dy2+8,'w':70,'h':44}]

            # Candles on floor framing the circle
            for (cx2,cy2) in [(W//2-160,H//2+50),(W//2+120,H//2+50),
                               (W//2-120,H//2-80),(W//2+80,H//2-80),
                               (W//2-60, H//2+90),(W//2+20, H//2+90)]:
                objs.append({'type':'candle','x':cx2,'y':cy2})

            # Magic circle — large, centred
            objs.append({'type':'magic_circle_floor','x':W//2,'y':H//2-10,'r':120})
        ...
        # ── BLACKSMITH ─────────────────────────────────────────────────────
            ...
            wpns_t=['axe','sword','axe','sword','axe','sword','axe','sword','axe','sword']
            ...
            wpns_r=['axe','sword','spear','axe','sword','spear','axe']
            for wi,wt in enumerate(wpns_r):
                objs.append({'type':'hung_weapon','x':W-ow-RW//2,'y':ow+RW+30+wi*spl,'weapon':wt,'orient':'h'})

            # Central forge fire pit — large
            fx,fy=W//2-60,H//2-10
            objs+=[{'type':'open_forge','x':fx,'y':fy,'r':90},conly(fx-95,fy-50,190,130)]

            # Weapons leaning on floor
            for (fpx,fpy,fw,fa) in [(fx+110,fy+60,'sword',0.35),(fx+135,fy+30,'axe',-0.2),
                                     (fx-115,fy+55,'sword',0.15),(fx-90,fy+35,'spear',0.05)]:
                objs.append({'type':'floor_weapon','x':fpx,'y':fpy,'weapon':fw,'angle':fa})

            # Anvil — large, bottom-left
            objs+=[{'type':'anvil','x':ow+RW+20,'y':H-ow-130},conly(ow+RW+10,H-ow-145,140,130)]
            # Coal pile near anvil
            objs.append({'type':'coal_pile','x':ow+RW+180,'y':H-ow-80})
        ...
        # ── ALCHEMIST ─────────────────────────────────────────────────────
            ...
            SW3=90  # shelf depth — prominent
            POT=['#ff4444','#4488ff','#ff8800','#44ff88','#ff44ff','#ffff44','#ff4444','#4488ff']
            # LEFT wall shelving unit with potions
            objs+=[{'type':'shelf_cabinet','x':ow,'y':ow,'w':SW3,'h':H-ow*2},conly(ow,ow,SW3,H-ow*2)]
            for pi in range((H-ow*2)//38): objs.append({'type':'potion_large','x':ow+SW3//2,'y':ow+19+pi*38,'color':POT[pi%8]})
            # RIGHT wall shelving unit
            objs+=[{'type':'shelf_cabinet','x':W-ow-SW3,'y':ow,'w':SW3,'h':H-ow*2},conly(W-ow-SW3,ow,SW3,H-ow*2)]
            for pi in range((H-ow*2)//38): objs.append({'type':'potion_large','x':W-ow-SW3//2,'y':ow+19+pi*38,'color':POT[(pi+4)%8]})
            # TOP wall shelving
            objs+=[{'type':'shelf_cabinet','x':ow+SW3,'y':ow,'w':W-ow*2-SW3*2,'h':SW3},conly(ow+SW3,ow,W-ow*2-SW3*2,SW3)]
            for pi in range((W-ow*2-SW3*2)//38): objs.append({'type':'potion_large','x':ow+SW3+19+pi*38,'y':ow+SW3//2,'color':POT[(pi+2)%8]})

            # 4 large cauldrons — positioned carefully
            ...
            # Worktable with mortar
            wx,wy=W-ow-SW3-180,ow+SW3+25
            objs+=[{'type':'worktable','x':wx,'y':wy,'w':160,'h':70,'collision':col(wx,wy,160,70)},
                   {'type':'mortar','x':wx+80,'y':wy-16},{'type':'candle','x':wx+145,'y':wy-8}]
        ...
        # ── TRADER ─────────────────────────────────────────────────────────
            ...
            BW=110  # back shelving depth
            # Full back wall shelving unit
            objs+=[{'type':'shelf_cabinet','x':ow,'y':ow,'w':W-ow*2,'h':BW},conly(ow,ow,W-ow*2,BW)]
            for ci in range((W-ow*2)//32):
                objs.append({'type':'shelf_item','x':ow+16+ci*32,'y':ow+BW//4,'color':['#cc4444','#4488ff','#cc8844','#44aa44','#aa44cc','#cccc44'][ci%6]})
                objs.append({'type':'shelf_item','x':ow+16+ci*32,'y':ow+BW*2//3,'color':['#44aa44','#cc4444','#4488ff','#cc8844','#aa44cc','#cccc44'][(ci+3)%6]})
            # Left wall shelves
            objs+=[{'type':'shelf_cabinet','x':ow,'y':ow+BW,'w':80,'h':H-ow*2-BW},conly(ow,ow+BW,80,H-ow*2-BW)]
            for ri in range(4):
                for ci2 in range(2):
                    objs.append({'type':'shelf_item','x':ow+20+ci2*40,'y':ow+BW+30+ri*((H-ow*2-BW-40)//4),'color':['#cc4444','#4488ff','#ff8800','#44aa44'][ri%4]})
            # Central L-counter
            cw3=W-ow-80-100
            objs+=[{'type':'shop_counter','x':ow+80,'y':H//2-30,'w':cw3,'h':55,'collision':col(ow+80,H//2-30,cw3,55)}]
            # Colourful goods on counter
            for gi2 in range(6):
                objs.append({'type':'shelf_item','x':ow+100+gi2*(cw3-40)//6,'y':H//2-46,'color':['#cc4444','#4488ff','#cc8844','#44aa44','#aa44cc','#44cccc'][gi2]})
            # Central map table
            tx,ty=W//2-70,H//2+55
            objs+=[{'type':'map_table','x':tx,'y':ty,'w':140,'h':80,'collision':col(tx,ty,140,80)},
                   {'type':'candle','x':tx+125,'y':ty-8}]
            # Barrels bottom-right
            for bi,(bx3,by3) in enumerate([(W-ow-75,H-ow-85),(W-ow-140,H-ow-70),(W-ow-105,H-ow-145)]):
                objs.append({'type':'barrel','x':bx3,'y':by3,'collision':col(bx3-28,by3-35,56,70)})
            # Crate stack bottom-left
            objs+=[{'type':'crate_stack','x':ow+90,'y':H-ow-130,'w':80,'h':120,'collision':col(ow+90,H-ow-130,80,120)}]

        # ── BAKERY ─────────────────────────────────────────────────────────
        elif btype == 'inn':
            divY=int(H*0.44); divX=int(W*0.58); dxg=int(W*0.22)
            walls+=[(ow,divY,dxg-dg//2,divY+iw),(dxg+dg//2,divY,W-ow,divY+iw),
                    (divX,divY+iw,divX+iw,H//2+H//8-dg//2),(divX,H//2+H//8+dg//2,divX+iw,H-ow)]
            for t,x,y in [('KITCHEN',W//2,ow+16),('COUNTER',(ow+divX)//2,divY+iw+16),
                           ('STORAGE',divX+(W-ow-divX)//2,divY+iw+16)]:
                objs.append(lbl(t,x,y,'#888888'))
            # Two large stone arch ovens side by side
            kitchen_w=W-ow*2-20
            oven_w=kitchen_w//2-8
            oven_h=divY-ow-25
            for oi in range(2):
                ox2=ow+10+oi*(oven_w+10)
                objs+=[{'type':'arch_oven','x':ox2,'y':ow+12,'w':oven_w,'h':oven_h,'collision':col(ox2,ow+12,oven_w,oven_h)}]
            # Items in kitchen
            objs+=[{'type':'barrel','x':ow+oven_w+5,'y':divY-ow-80,'collision':col(ow+oven_w-23,divY-ow-115,50,80)},
                   {'type':'sack','x':W-ow-65,'y':divY-ow-75,'collision':col(W-ow-95,divY-ow-110,60,75)}]
            # Counter section
            ctr_w=divX-ow-65
            objs+=[{'type':'bakery_counter','x':ow+20,'y':divY+iw+50,'w':ctr_w,'h':60,'collision':col(ow+20,divY+iw+50,ctr_w,60)},
                   {'type':'bread_display','x':ow+35,'y':divY+iw+32},
                   {'type':'dining_table','x':ow+35,'y':divY+iw+165,'w':100,'h':65,'collision':col(ow+35,divY+iw+165,100,65)},
                   {'type':'chair','x':ow+50,'y':divY+iw+237,'w':40,'h':40,'collision':col(ow+50,divY+iw+237,40,40)},
                   {'type':'candle','x':ow+80,'y':divY+iw+158}]
            # Storage
            for bi2,(bx4,by4,bl4) in enumerate([(divX+iw+30,divY+iw+60,'Honey'),(divX+iw+105,divY+iw+60,'Salt'),(divX+iw+180,divY+iw+60,'Milk')]):
                objs+=[{'type':'barrel','x':bx4,'y':by4,'collision':col(bx4-28,by4-35,56,70)},lbl(bl4,bx4,by4+44,'#ccbb99')]
            stg_sh_x,stg_sh_y=divX+iw+20,divY+iw+130
            stg_sh_w=W-ow-divX-iw-25
            objs.append({'type':'wall_shelf','x':stg_sh_x,'y':stg_sh_y,'w':stg_sh_w,'h':18,'collision':col(stg_sh_x,stg_sh_y,stg_sh_w,18)})
            for si3 in range(2):
                objs.append({'type':'sack','x':divX+iw+30+si3*100,'y':H-ow-80,'collision':col(divX+iw+2+si3*100,H-ow-115,56,75)})
            ...
            if self.current_interior:
                ...
                for obj in objs:
                    ...
                    # ── BOOKCASE UNIT — the detailed shelving ─────────
                        ...
                        # Wooden frame
                        cv.create_rectangle(ox,oy,ox+ow2,oy+oh2,fill='#4a2e0e',outline='#2a1808',width=3)
                        # Number of shelves depends on orientation
                        if side in ('left','right') or ot=='shelf_cabinet':
                            shelf_spacing=max(28,oh2//8)
                            for shi in range(oh2//shelf_spacing):
                                sy=oy+shi*shelf_spacing
                                cv.create_rectangle(ox+2,sy,ox+ow2-2,sy+shelf_spacing-2,fill='#3a2208',outline='')
                                cv.create_rectangle(ox+2,sy+shelf_spacing-8,ox+ow2-2,sy+shelf_spacing-2,fill='#5a3810',outline='')
                                # Books on shelf
                                bx=ox+4
                                while bx < ox+ow2-8:
                                    bw=max(8,min(16,(ox+ow2-8-bx)))
                                    bh=max(14,shelf_spacing-14)
                                    bc=['#8B1A1A','#1A3A8B','#1A6B1A','#8B7A1A','#6B1A6B','#1A6B6B','#8B4A1A','#4A1A8B'][(shi*7+bx)%8]
                                    cv.create_rectangle(bx,sy+4,bx+bw,sy+4+bh,fill=bc,outline='#111',width=1)
                                    # Book title line
                                    if bw>10:
                                        cv.create_line(bx+2,sy+8,bx+bw-2,sy+8,fill='#888888',width=1)
                                    bx+=bw+2
                        else:  # top shelf
                            col_spacing=max(28,ow2//12)
                            for shi in range(ow2//col_spacing):
                                sx=ox+shi*col_spacing
                                cv.create_rectangle(sx,oy+2,sx+col_spacing-2,oy+oh2-2,fill='#3a2208',outline='')
                                cv.create_rectangle(sx,oy+oh2-8,sx+col_spacing-2,oy+oh2-2,fill='#5a3810',outline='')
                                # Vertical books
                                by2=oy+4
                                while by2 < oy+oh2-12:
                                    bh2=max(8,min(16,(oy+oh2-12-by2)))
                                    bw2=col_spacing-6
                                    bc2=['#8B1A1A','#1A3A8B','#1A6B1A','#8B7A1A','#6B1A6B','#1A6B6B'][(shi+by2)%6]
                                    cv.create_rectangle(sx+3,by2,sx+3+bw2,by2+bh2,fill=bc2,outline='#111',width=1)
                                    by2+=bh2+2
                        ...
                        r=obj.get('r',90)
                        # Stone ring — outer
                        cv.create_oval(ox-r,oy-r//2,ox+r,oy+r//2,fill='#3a3020',outline='#555545',width=5)
                        # Stone blocks in ring
                        for si2 in range(12):
                            a=si2*math.pi/6
                            sx2=ox+int(math.cos(a)*r*0.88); sy2=oy+int(math.sin(a)*r//2*0.88)
                            cv.create_oval(sx2-8,sy2-6,sx2+8,sy2+6,fill='#4a4030',outline='#2a2820',width=1)
                        # Inner fire bed
                        cv.create_oval(ox-r+14,oy-r//2+10,ox+r-14,oy+r//2-10,fill='#1a0800',outline='')
                        # Glowing embers
                        for ei in range(10):
                            a=now*0.5+ei*0.628
                            ex2=ox+int(math.cos(a)*(r-28)); ey2=oy+int(math.sin(a)*((r//2)-16))
                            ec=['#ff3300','#ff5500','#ff8800','#ffaa00'][ei%4]
                            cv.create_oval(ex2-5,ey2-4,ex2+5,ey2+4,fill=ec,outline='')
                        # Multiple fire tongues
                        for fi in range(6):
                            fx2=ox+int(math.cos(fi*1.047)*(r-38)); fy2=oy+int(math.sin(fi*1.047)*((r//2)-22))
                            fire(fx2,fy2,fw=30,fh=45,t=now+fi*0.25)
                        ...
                        cv.create_oval(ox-28,ox-16,ox+28,oy+30,fill='#1a1a1a',outline='#444',width=3)
                        cv.create_oval(ox-24,oy-12,ox+24,oy+18,fill=lc3,outline='')
                        for bi3 in range(4):
                            ph=(now*0.9+bi3*0.25)%1.0
                            bubble(ox-12+bi3*8,oy+6,ph,col='#88ffaa')
