...
class GameFrame(tk.Frame):
    ...
    def interact_with_npc(self, npc):
        ...
        SHOP_CLOSE_DISTANCE = 160   # pixels — same as interaction radius × 2

        def _npc_x():
            """Current screen X of the NPC (world coords for indoor, screen for outdoor)."""
            return npc.x
        def _npc_y():
            return npc.y
        ...
        def _check_distance():
            try:
                if not shop_win.winfo_exists():
                    return
                dist = math.hypot(self.player.x - _npc_x(),
                                  self.player.y - _npc_y())
        ...
        if self.dungeon_id == 0:
            ...
            if hasattr(self.player, '_orbit_blades') and self.player._orbit_blades:
                for blade in self.player._orbit_blades:
                    if not blade.get('launched', True):
