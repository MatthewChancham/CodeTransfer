...
class Player:
    ...
    def __init__(self,name='Hero',class_name='Warrior'):
        ...
        if class_name == 'Monk':
            ...
            self.strength=500; self.vitality=50000; self.agility=500
    ...
    def populate_skills(self):
        ...
        def fire_storm(player, game):
            """Ignis's flame swirl — three rotating fire rings damage nearby enemies for 4s. CD: 30s."""
            if player.mana < 40:
                return
            player.mana -= 40
...
class Item:
    ...
    def draw_ignis_staff(self, canvas):
        """Gold fire-staff: angular shaft, spear tip, prong cradle, flame-particle tip."""
        import time as _time
        now = _time.time()

        staff_len = self.size * 3.2
        fwd = 6
        cx = self.x + math.cos(self.angle) * fwd
        cy = self.y + math.sin(self.angle) * fwd
        ...
        # ── Flame radius (pulsing) ──
        pulse   = abs(math.sin(now * 5.0))
        flame_r = 7 + pulse * 3

        # ── Spear tip ──
        tip_len = self.size * 1.2
        tip_w   = self.size * 0.26
        tip_base = (hx, hy)
        tip_top  = (hx + ca * tip_len,           hy + sa * tip_len)
        tip_l    = (hx + pa*tip_w - ca*(tip_len*0.35), hy + ps*tip_w - sa*(tip_len*0.35))
        tip_r    = (hx - pa*tip_w - ca*(tip_len*0.35), hy - ps*tip_w - sa*(tip_len*0.35))
        canvas.create_polygon(tip_base[0]+2, tip_base[1]+2,
                               tip_l[0]+2, tip_l[1]+2,
                               tip_top[0]+2, tip_top[1]+2,
                               tip_r[0]+2, tip_r[1]+2,
                               fill='#3a2800', outline='')
        canvas.create_polygon(tip_base[0], tip_base[1],
                               tip_l[0], tip_l[1],
                               tip_top[0], tip_top[1],
                               tip_r[0], tip_r[1],
                               fill='#B8860B', outline='#FFD700', width=2)
        ti_l = (hx + pa*(tip_w*0.5) - ca*(tip_len*0.3), hy + ps*(tip_w*0.5) - sa*(tip_len*0.3))
        ti_r = (hx - pa*(tip_w*0.5) - ca*(tip_len*0.3), hy - ps*(tip_w*0.5) - sa*(tip_len*0.3))
        canvas.create_polygon(hx, hy, ti_l[0], ti_l[1],
                               tip_top[0], tip_top[1], ti_r[0], ti_r[1],
                               fill='#FFD700', outline='')

        # ── Side prongs cradling the flame ──
        prong_start_fwd = tip_len * 0.30
        prong_start_w   = tip_w * 1.2
        prong_mid_fwd   = tip_len * 0.62
        prong_mid_w     = flame_r + 6
        prong_tip_fwd   = tip_len * 1.05
        prong_tip_w     = flame_r * 0.7
        for side in (-1, 1):
            p0x = hx + ca*prong_start_fwd + pa*side*prong_start_w
            p0y = hy + sa*prong_start_fwd + ps*side*prong_start_w
            p1x = hx + ca*prong_mid_fwd   + pa*side*prong_mid_w
            p1y = hy + sa*prong_mid_fwd   + ps*side*prong_mid_w
            p2x = hx + ca*prong_tip_fwd   + pa*side*prong_tip_w
            p2y = hy + sa*prong_tip_fwd   + ps*side*prong_tip_w
        ...
        # ── Flame tip — teardrop matching engine flame particle, base at shaft end ──
        fl_r   = flame_r
        fl_l   = (hx - pa*fl_r, hy - ps*fl_r)
        fl_r2  = (hx + pa*fl_r, hy + ps*fl_r)
        fl_tip = (hx + ca*(fl_r*2.5), hy + sa*(fl_r*2.5))
        fl_col = 'orange' if (int(now*12) % 2 == 0) else 'yellow'
        canvas.create_polygon(fl_l[0], fl_l[1], fl_r2[0], fl_r2[1],
                               fl_tip[0], fl_tip[1], fill=fl_col, outline='')
        ic_cx = hx + ca*(fl_r*0.5); ic_cy = hy + sa*(fl_r*0.5); ic_r = fl_r*0.6
        canvas.create_oval(ic_cx-ic_r, ic_cy-ic_r, ic_cx+ic_r, ic_cy+ic_r,
                           fill='yellow', outline='')

        # ── Angular pommel ──
        px2 = bx - ca*5; py2 = by - sa*5
        pts_pom = [px2+pa*4,py2+ps*4, px2+ca*7,py2+sa*7,
                   px2-pa*4,py2-ps*4, px2-ca*7,py2-sa*7]
...
class Boss(Enemy):
    ...
    def _update_ignis_phase4(self, dt, game, now, player):
        ...
        # Flee directly away from player, with slight erratic wobble
    ...
    def _ignis_fire_breath(self, game, breath_range, dense=False):
        ...
        if not dense:
            ...
            # ── Phase 2/3: scorching ray — random scatter so no dotted gaps ──
            perp_ang = ang + math.pi / 2
            for _ in range(10):
...
class SkillTreeWindow:
    ...
    CW, CH    = 1020, 640
    TIER_Y    = {1: 65, 2: 195, 3: 325, 4: 455, 5: 570,
                 6: 715, 7: 845, 8: 975}
    ...
    def _draw(self):
        ...
        c.create_text(self.BRANCH_X['right'], 22, text="— PASSIVE SKILLS —",
                      font=("Arial", 10, "bold"), fill='#335566')
        # Extra branch label (Chain Lightning for Mage)
...
class GameFrame(tk.Frame):
    ...
    def on_key_down(self, e):
        ...
        # 6 cycles skill pages (Keen Mind = 2 pages, Cognitive Expansion = 3 pages)
        if e.keysym == '6':
            p = self.player
    ...
    def _draw_skill_hotbar(self):
        ...
        page   = getattr(p, 'skill_page', 1)
        offset = (page - 1) * 5   # page 1→0, page 2→5, page 3→10

        # Page indicator (only if Keen Mind unlocked and toggled on)
        has_keen = ('Keen Mind' in getattr(p, 'tree_unlocked', set())
                    and p.passive_toggles.get('Keen Mind', True))
        if has_keen:
        ...
        for sk in p.unlocked_skills:
            ...
            if offset + 1 <= k <= offset + 5:
                slot_skill[k - offset] = sk   # map to display slot 1-5
    ...
    def update_player(self,dt):
        ...
        # ── Fire Storm: three rotating flame rings around the player ──────────
        if getattr(p, '_fire_storm_end', 0) > now:
            ...
            for (_sr, _rot, _count, _lmin, _lmax) in [
                ...
                for _si in range(_count):
                    ...
                    _px = p.x + math.cos(_sa) * (_sr + random.uniform(-6, 6))
                    _py = p.y + math.sin(_sa) * (_sr + random.uniform(-6, 6))
                    fl = Particle(_px, _py,
                                  random.uniform(6, 13),
                                  random.choice(['#ff2200','#ff6600','#ffaa00','#ffcc00','#ffff44']),
                                  life=random.uniform(_lmin, _lmax),
                                  rtype='flame', owner='player')
    ...
    # Continue with summons drawing...
        ...
        for e in self.room.enemies:
            ...
            if getattr(e, '_is_ignismancer', False):
                ...
                _staff_item = Item(ex, ey, 'ignis_staff', '#ff4400', 14)
                ...
                if e.boss_type == 'FireLord':
                    ...
                    if _ig_phase == 4:
                        ...
                        if _analysed:
                            ...
                            # No HP bar in phoenix form — just revive timer
                    else:
