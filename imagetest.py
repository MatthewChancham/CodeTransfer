    ...
    def _gs_do_rapid_swing(self, dt, game, now, player):
        ...
        while self.gs_swing_angle >= self.gs_rapid_stage * threshold + threshold:
            ...
            if self.gs_rapid_stage > 9:   # max 9 bursts in 2.4s
                break
            ang = math.atan2(player.y - self.y, player.x - self.x)
            # 3 slashes per burst: direct + slight spread
            for delta in [-0.25, 0.0, 0.25]:
                game.spawn_projectile(self.x, self.y, ang + delta, 13, 3.0, 30,
                                      '#888888', self.atk * 2.0, 'enemy', stype='slash')
...
def spawn_boss_for_room(room, dungeon_id):
    ...
    if dungeon_id == 1:
        ...
        totem_hp = int(1000 * (1.2 ** (player_level - 1)))
    ...
    # Continue with summons drawing...
        ...
        for proj in self.projectiles:
            ...
            if proj.stype == 'arrow':
                ...
                # Flying greatsword — rectangular blade + tip, handle, crossguard
                _a  = proj.angle
                _ca = math.cos(_a); _sa = math.sin(_a)
                _pa = math.cos(_a + math.pi/2); _ps = math.sin(_a + math.pi/2)
                _z  = r * 1.0   # scale
                # Blade rectangle: 4 corners
                _bl = _z * 4.0   # blade length
                _bw = _z * 0.45  # blade half-width
                _tl = _z * 0.8   # tip length
                # Rectangle base corners (at x,y = crossguard position)
                _r1x = x + _pa*_bw;      _r1y = y + _ps*_bw
                _r2x = x - _pa*_bw;      _r2y = y - _ps*_bw
                _r3x = x+_ca*_bl-_pa*_bw*0.3; _r3y = y+_sa*_bl-_ps*_bw*0.3
                _r4x = x+_ca*_bl+_pa*_bw*0.3; _r4y = y+_sa*_bl+_ps*_bw*0.3
                _tx  = x + _ca*(_bl+_tl); _ty  = y + _sa*(_bl+_tl)
                # Blade body rectangle
                self.canvas.create_polygon([_r1x,_r1y, _r4x,_r4y, _r3x,_r3y, _r2x,_r2y],
                                           fill='#787878', outline='#2a2a2a', width=2)
                # Centre highlight
                self.canvas.create_line(x+_pa*_bw*0.15, y+_ps*_bw*0.15,
                                        x+_ca*_bl+_pa*_bw*0.15, y+_sa*_bl+_ps*_bw*0.15,
                                        fill='#d0d0d0', width=2)
                # Pointed tip
                self.canvas.create_polygon([_r4x,_r4y, _tx,_ty, _r3x,_r3y],
                                           fill='#999999', outline='#2a2a2a', width=1)
                self.canvas.create_line(_r4x,_r4y, _tx,_ty, fill='white', width=1)
                # Handle (behind origin)
                _hx = x - _ca*_z*1.5; _hy = y - _sa*_z*1.5
                self.canvas.create_line(x, y, _hx, _hy, fill='#3d1f0a', width=5)
                # Crossguard
                _cx1 = x+_pa*_z*1.3; _cy1 = y+_ps*_z*1.3
                _cx2 = x-_pa*_z*1.3; _cy2 = y-_ps*_z*1.3
