...
class CasinoApp(tk.Tk):
    def __init__(self):
        ...
        self.figurine_display_rooms={}  # {room_id: [fid,...]} per-room display list
    ...
    def _key_down(self,e):
        ...
        if e.keysym=="Escape":
            if getattr(self,"_esc_blocked",False): return
        ...
        elif e.keysym.lower()=="o":
            self._toggle_wardrobe()
        elif e.keysym.lower()=="c" and self.screen=="interior":
            room=self.int_rooms.get(self.int_room,{})
            if room.get("has_tv"):
                # Only watch TV when close to TV position (top-right of room)
                tv_x=room.get("tv_x",W-120); tv_y=room.get("tv_y",260)
                if math.hypot(self.int_px-tv_x, self.int_py-tv_y)<130:
        ...
        elif e.keysym=="k" and getattr(self,"creative_mode",False):
            self.boss_alert_level=2; self.debt=max(self.debt,500); self._boss_encounter()
        elif e.keysym=="l" and getattr(self,"creative_mode",False):
    ...
    # ── WARDROBE OVERLAY (O key) — uses Toplevel so it survives c.delete("all") ──
    def _toggle_wardrobe(self):
        win=getattr(self,"_ward_win",None)
        if win and win.winfo_exists():
            win.destroy(); self._ward_win=None; return
        self._open_wardrobe(subtab=getattr(self,"_ward_last_tab","skin"))

    def _open_wardrobe(self,subtab="skin"):
        self._ward_last_tab=subtab
        ...
        win=tk.Toplevel(self); self._ward_win=win
        win.overrideredirect(True)
        self.update_idletasks()
        rx=self.winfo_rootx(); ry=self.winfo_rooty()
        win.geometry(f"{PW}x{PH}+{rx+PX}+{ry+PY}")
        win.configure(bg="#0d0d14")
        win.lift()
        # Bind O on the toplevel so it still closes when toplevel has focus
        win.bind("<KeyPress-o>",lambda e:self._toggle_wardrobe())
        win.bind("<KeyPress-O>",lambda e:self._toggle_wardrobe())

        TABS=["skin","card","dice","figurines"]
        TAB_LABELS={"skin":"Player Skin","card":"Card Skin","dice":"Dice Skin","figurines":"Figurines"}

        # ── Header ────────────────────────────────────────
        hdr=tk.Frame(win,bg="#0d0d14"); hdr.pack(fill="x",padx=4,pady=(4,0))
        tk.Label(hdr,text="✦  WARDROBE  ✦  [O] to close",bg="#0d0d14",fg=GOLD,
                 font=tkfont.Font(family="Georgia",size=12,weight="bold")).pack(side="left",padx=8)
        tk.Button(hdr,text="✕",command=lambda:self._toggle_wardrobe(),
                  bg="#1a0010",fg="#ff6666",font=tkfont.Font(size=10,weight="bold"),
                  relief="flat",cursor="hand2").pack(side="right",padx=4)

        # ── Tab bar ───────────────────────────────────────
        tab_bar=tk.Frame(win,bg="#0d0d14"); tab_bar.pack(fill="x",padx=4,pady=2)
        for tab in TABS:
            active=tab==subtab
            bg_c="#2a1a40" if active else "#0d0920"
            fg_c=GOLD if active else "#666"
            relief_="flat"
            btn=tk.Button(tab_bar,text=TAB_LABELS[tab],bg=bg_c,fg=fg_c,
                          font=tkfont.Font(family="Courier New",size=9,weight="bold"),
                          relief=relief_,cursor="hand2",padx=8,pady=4,
                          command=lambda t=tab:self._open_wardrobe(t))
            btn.pack(side="left",padx=2)

        # ── Scrollable content area ───────────────────────
        frame_outer=tk.Frame(win,bg="#0a0a12"); frame_outer.pack(fill="both",expand=True,padx=4,pady=4)
        vsb=tk.Scrollbar(frame_outer,orient="vertical",bg="#222",troughcolor="#111")
        vsb.pack(side="right",fill="y")
        canvas2=tk.Canvas(frame_outer,bg="#0a0a12",highlightthickness=0,yscrollcommand=vsb.set)
        canvas2.pack(side="left",fill="both",expand=True)
        vsb.config(command=canvas2.yview)
        inner=tk.Frame(canvas2,bg="#0a0a12")
        inner_id=canvas2.create_window(0,0,anchor="nw",window=inner)

        def _on_configure(e):
            canvas2.configure(scrollregion=canvas2.bbox("all"))
            canvas2.itemconfig(inner_id,width=canvas2.winfo_width())
        inner.bind("<Configure>",_on_configure)
        canvas2.bind("<Configure>",lambda e:canvas2.itemconfig(inner_id,width=e.width))

        def _on_wheel(e):
            dy=-1 if (e.delta>0 or e.num==4) else 1
            canvas2.yview_scroll(dy,"units")
        canvas2.bind("<MouseWheel>",_on_wheel)
        canvas2.bind("<Button-4>",_on_wheel)
        canvas2.bind("<Button-5>",_on_wheel)
        inner.bind("<MouseWheel>",_on_wheel)

        CELL=130; COLS=5

        def add_skin_tab():
            for idx,(sid,sk) in enumerate(SKINS.items()):
                col_=idx%COLS; row_=idx//COLS
                owned_s=sid in getattr(self,"owned_skins",{"default"})
                cell=tk.Frame(inner,bg="#0d0d14",bd=2,relief="solid",
                              highlightbackground=GOLD if self.equipped_skin==sid else ("#335533" if owned_s else "#333"),
                              highlightthickness=2)
                cell.grid(row=row_,column=col_,padx=6,pady=6,ipadx=4,ipady=4)
                # Mini player drawn on a tiny canvas
                pc=tk.Canvas(cell,width=60,height=64,bg="#0d0d14",highlightthickness=0)
                pc.pack()
                px2,py2=30,36
                pc.create_oval(px2-5,py2+10,px2+5,py2+15,fill="#0a0000",outline="")
                pc.create_rectangle(px2-4,py2+3,px2-1,py2+12,fill=sk["legs"],outline="")
                pc.create_rectangle(px2+1,py2+3,px2+4,py2+12,fill=sk["legs"],outline="")
                pc.create_rectangle(px2-7,py2-4,px2+7,py2+5,fill=sk["body"],outline="")
                pc.create_rectangle(px2-10,py2-3,px2-7,py2+3,fill=sk["body"],outline="")
                pc.create_rectangle(px2+7,py2-3,px2+10,py2+3,fill=sk["body"],outline="")
                pc.create_oval(px2-6,py2-14,px2+6,py2-5,fill=sk["face"],outline="")
                pc.create_rectangle(px2-8,py2-16,px2+8,py2-14,fill=sk["hat"],outline="")
                pc.create_rectangle(px2-5,py2-22,px2+5,py2-16,fill=sk["hat"],outline="")
                pc.create_line(px2-5,py2-19,px2+5,py2-19,fill=sk["stripe"],width=1)
                equipped=self.equipped_skin==sid
                owned=sid in getattr(self,"owned_skins",{"default"})
                name_lbl=tk.Label(cell,text=sk["name"],bg="#0d0d14",
                                  fg=GOLD if equipped else (CREAM if owned else "#555"),
                                  font=tkfont.Font(family="Courier New",size=8,weight="bold"))
                name_lbl.pack()
                if equipped:
                    tk.Label(cell,text="EQUIPPED",bg="#0d0d14",fg=GREEN_C,
                             font=tkfont.Font(family="Courier New",size=7)).pack()
                elif owned:
                    def _equip(e,s=sid):
                        self.equipped_skin=s; self._open_wardrobe("skin")
                    tk.Label(cell,text="owned — click to equip",bg="#0d0d14",fg="#5a8a5a",
                             font=tkfont.Font(family="Courier New",size=6)).pack()
                    for w2 in [cell,pc,name_lbl]:
                        w2.bind("<Button-1>",_equip)
                else:
                    tk.Label(cell,text=f"🔒 ${sk['price']:,}",bg="#0d0d14",fg="#444",
                             font=tkfont.Font(family="Courier New",size=7)).pack()

        def add_card_tab():
            for idx,(cid,ck) in enumerate(CARD_SKINS.items()):
                col_=idx%COLS; row_=idx//COLS
                equipped=getattr(self,"equipped_card_skin","classic")==cid
                owned=cid in getattr(self,"owned_card_skins",{"classic"})
                cell=tk.Frame(inner,bg="#0d0d14",bd=2,relief="solid",
                              highlightbackground=GOLD if equipped else ("#335533" if owned else "#333"),
                              highlightthickness=2)
                cell.grid(row=row_,column=col_,padx=6,pady=6,ipadx=4,ipady=4)
                pc=tk.Canvas(cell,width=60,height=64,bg="#0d0d14",highlightthickness=0)
                pc.pack()
                cx2,cy2=30,34
                pc.create_rectangle(cx2-14,cy2-20,cx2+14,cy2+20,fill=ck["face"],outline=ck["border"],width=2)
                pc.create_rectangle(cx2-11,cy2-17,cx2+11,cy2+17,fill=ck["back"],outline="")
                pc.create_text(cx2,cy2,text="A♠",fill=ck["sym_b"],
                               font=tkfont.Font(family="Georgia",size=10,weight="bold"))
                name_lbl=tk.Label(cell,text=ck["name"],bg="#0d0d14",
                                  fg=GOLD if equipped else (CREAM if owned else "#555"),
                                  font=tkfont.Font(family="Courier New",size=8,weight="bold"))
                name_lbl.pack()
                if equipped:
                    ...
                    def _equip(e,s=cid):
                        self.equipped_card_skin=s; self._open_wardrobe("card")
                    tk.Label(cell,text="owned — click to equip",bg="#0d0d14",fg="#5a8a5a",
                             font=tkfont.Font(family="Courier New",size=6)).pack()
                    for w2 in [cell,pc,name_lbl]:
                        w2.bind("<Button-1>",_equip)
                else:
                    tk.Label(cell,text=f"🔒 ${ck['price']:,}",bg="#0d0d14",fg="#444",
                             font=tkfont.Font(family="Courier New",size=7)).pack()

        def add_dice_tab():
            for idx,(did,dk) in enumerate(DICE_SKINS.items()):
                col_=idx%COLS; row_=idx//COLS
                equipped=getattr(self,"equipped_dice_skin","classic")==did
                owned=did in getattr(self,"owned_dice_skins",{"classic"})
                cell=tk.Frame(inner,bg="#0d0d14",bd=2,relief="solid",
                              highlightbackground=GOLD if equipped else ("#335533" if owned else "#333"),
                              highlightthickness=2)
                cell.grid(row=row_,column=col_,padx=6,pady=6,ipadx=4,ipady=4)
                ...
                pc.pack()
                dx2,dy2=30,34
                pc.create_rectangle(dx2-18,dy2-18,dx2+18,dy2+18,fill=dk["col"],outline=dk["ol"],width=2)
                for dpx,dpy in [(-8,-8),(8,-8),(0,0),(-8,8),(8,8)]:
                    pc.create_oval(dx2+dpx-3,dy2+dpy-3,dx2+dpx+3,dy2+dpy+3,fill=dk["dot"],outline="")
                name_lbl=tk.Label(cell,text=dk["name"],bg="#0d0d14",
                                  fg=GOLD if equipped else (CREAM if owned else "#555"),
                                  font=tkfont.Font(family="Courier New",size=8,weight="bold"))
                name_lbl.pack()
                if equipped:
                    ...
                    def _equip(e,s=did):
                        self.equipped_dice_skin=s; self._open_wardrobe("dice")
                    tk.Label(cell,text="owned — click to equip",bg="#0d0d14",fg="#5a8a5a",
                             font=tkfont.Font(family="Courier New",size=6)).pack()
                    for w2 in [cell,pc,name_lbl]:
                        w2.bind("<Button-1>",_equip)
                else:
                    tk.Label(cell,text=f"🔒 ${dk['price']:,}",bg="#0d0d14",fg="#444",
                             font=tkfont.Font(family="Courier New",size=7)).pack()

        def add_figurines_tab():
            coll=getattr(self,"figurine_collection",[])
            if not coll:
                tk.Label(inner,text="No figurines yet!\nVisit the Gumball Emporium.",
                         bg="#0a0a12",fg="#666",font=tkfont.Font(family="Courier New",size=10),
                         justify="center").grid(row=0,column=0,columnspan=COLS,pady=40)
                return
            seen={}
            for fid in coll: seen[fid]=seen.get(fid,0)+1
            unique=list(seen.items())
            for idx,(fid,count) in enumerate(unique):
                if fid not in self.FIGURINE_BY_ID: continue
                _,name,rarity,_,desc=self.FIGURINE_BY_ID[fid]
                rc=self.RARITY_COLS.get(rarity,"#aaa")
                col_=idx%COLS; row_=idx//COLS
                cell=tk.Frame(inner,bg="#100c18",bd=2,relief="solid",
                              highlightbackground=rc,highlightthickness=1)
                cell.grid(row=row_,column=col_,padx=5,pady=5,ipadx=3,ipady=3)
                # Draw figurine on small canvas
                fc=tk.Canvas(cell,width=60,height=64,bg="#100c18",highlightthickness=0)
                fc.pack()
                self._draw_figurine(fc,fid,30,32,size=28)
                tk.Label(cell,text=name,bg="#100c18",fg=CREAM,wraplength=90,
                         font=tkfont.Font(family="Courier New",size=7,weight="bold")).pack()
                tk.Label(cell,text=rarity.upper(),bg="#100c18",fg=rc,
                         font=tkfont.Font(family="Courier New",size=6)).pack()
                if count>1:
                    tk.Label(cell,text=f"×{count}",bg="#100c18",fg=GOLD,
                             font=tkfont.Font(family="Courier New",size=7,weight="bold")).pack()

        if subtab=="skin": add_skin_tab()
        elif subtab=="card": add_card_tab()
        elif subtab=="dice": add_dice_tab()
        elif subtab=="figurines": add_figurines_tab()
    ...
    def _draw_interior(self):
        ...
        if self.int_building=="hotel":
            ...
            elif self.int_room.startswith("room_"):
                # Three display shelves per room, each with their own figurine list
                shelf_ys_r=[290,390,490]
                is_f3=self.int_room.startswith("room_2")
                max_s=6 if is_f3 else 4
                slot_gap=28
                for ti,shelf_y in enumerate(shelf_ys_r):
                    tbl_key=f"{self.int_room}_t{ti}"
                    disp=getattr(self,"figurine_display_rooms",{}).get(tbl_key,[])
                    sx_base=W//2+220
                    for si,fid in enumerate(disp[:max_s]):
                        gx=sx_base-(max_s-1)*slot_gap//2+si*slot_gap
                        self._draw_figurine(c,fid,gx,shelf_y-14,size=22)
    ...
    def _check_npc_proximity(self):
        ...
        if not self.nearby_npc:
            # Also check figurine display tables (no sprite NPC, just proximity)
            for tbl in room.get("fig_tables",[]):
                tx,ty=tbl["x"],tbl["y"]
                if math.hypot(self.int_px-tx,self.int_py-ty)<110:
                    self.nearby_npc=tbl; break
    ...
    def _try_interact(self):
        ...
        game_map={
            ...
            **{f"fig_room_{rk}_{ti}":( lambda rk2=rk,t2=ti: self._figurine_room_screen(rk2,table_idx=t2) )
               for rk in [f"room_{101+i}" for i in range(8)]+[f"room_{201+i}" for i in range(8)]
               for ti in range(3)},
        }
    ...
    def _encode_save(self):
        state={
            ...
            "figurine_display_rooms":getattr(self,"figurine_display_rooms",{}),
        }
    ...
    def _decode_save(self,code):
        try:
            ...
            self.figurine_display_rooms=dict(state.get("figurine_display_rooms",getattr(self,"figurine_display_rooms",{})))
            return True
        except Exception:
            return False
    ...
    def _draw_figurine(self,c,fid,x,y,size=28):
        """Draw a unique detailed sprite for each figurine."""
        if fid not in self.FIGURINE_BY_ID: return
        _,name,rarity,col,_ = self.FIGURINE_BY_ID[fid]
        rc=self.RARITY_COLS[rarity]; s=size//2
        # Rarity glow ring
        c.create_oval(x-s-3,y-s-3,x+s+3,y+s+3,fill="",outline=rc,width=2)
        k=max(1,s//14)  # line width scaler

        if fid=="f_coin":
            # Gold coin with etched face
            c.create_oval(x-s,y-s,x+s,y+s,fill="#c8a000",outline="#8a6000",width=k+1)
            c.create_oval(x-s+2,y-s+2,x+s-2,y+s-2,fill="#e8c818",outline="#c8a800",width=k)
            c.create_oval(x-s+4,y-s+4,x+s-4,y+s-4,fill="",outline="#a07800",width=k)
            c.create_text(x,y,text="$",fill="#7a5800",font=tkfont.Font(family="Georgia",size=max(6,s),weight="bold"))
            for ri_ in range(0,360,30):
                ra_=math.radians(ri_)
                c.create_line(x+int((s-2)*math.cos(ra_)),y+int((s-2)*math.sin(ra_)),
                              x+int(s*math.cos(ra_)),y+int(s*math.sin(ra_)),fill="#a07800",width=k)

        elif fid=="f_dice":
            # White die with rounded corners and pips
            c.create_rectangle(x-s,y-s,x+s,y+s,fill="#f0f0f0",outline="#999",width=k+1)
            c.create_rectangle(x-s+2,y-s+2,x+s-2,y+s-2,fill="#ffffff",outline="#ccc",width=k)
            # pips in 3 pattern (showing 3 face)
            p=max(2,s//5)
            for px_,py_ in [(-s//2,-s//2),(0,0),(s//2,s//2)]:
                c.create_oval(x+px_-p,y+py_-p,x+px_+p,y+py_+p,fill="#222",outline="")
            # Corner highlights
            for cx_,cy_ in [(-s+2,-s+2),(s-2,-s+2)]:
                c.create_line(x+cx_,y+cy_,x+cx_+4,y+cy_,fill="#eee",width=k)

        elif fid=="f_card":
            # Playing card with suit and number
            c.create_rectangle(x-s+2,y-s,x+s-2,y+s,fill="#fffef8",outline="#333",width=k)
            c.create_rectangle(x-s+4,y-s+2,x+s-4,y+s-2,fill="#fffef8",outline="#ccc",width=k)
            c.create_text(x-s+6,y-s+6,text="A",fill="#111",font=tkfont.Font(size=max(4,s//2),weight="bold"),anchor="nw")
            c.create_text(x,y,text="♠",fill="#111",font=tkfont.Font(size=max(7,s),weight="bold"))
            c.create_text(x+s-6,y+s-6,text="A",fill="#111",font=tkfont.Font(size=max(4,s//2),weight="bold"),anchor="se")

        elif fid=="f_horseshoe":
            # Detailed horseshoe with nail holes
            c.create_arc(x-s,y-s//2,x+s,y+s+s//2,start=0,extent=180,outline=col,style="arc",width=max(4,s//3))
            c.create_line(x-s,y+s//2,x-s,y+s,fill=col,width=max(4,s//3))
            c.create_line(x+s,y+s//2,x+s,y+s,fill=col,width=max(4,s//3))
            # Nail holes
            for nx_,ny_ in [(-s//2,s//4),(0,-s//3),(s//2,s//4)]:
                c.create_oval(x+nx_-2,y+ny_-2,x+nx_+2,y+ny_+2,fill="#555",outline="#333",width=1)
            # Shine
            c.create_arc(x-s+2,y-s//2+2,x+s-2,y+s+s//2-2,start=20,extent=40,outline="#dddddd",style="arc",width=k+1)

        elif fid=="f_mug":
            # Beer mug with foam and handle
            c.create_rectangle(x-s+2,y-s//3,x+s-2,y+s,fill="#d4a030",outline="#9a7010",width=k+1)
            # Foam
            for fx_ in range(x-s+3,x+s-3,5):
                c.create_oval(fx_,y-s//3-4,fx_+6,y-s//3+4,fill="#fffef0",outline="#e8e0c0",width=1)
            # Handle
            c.create_arc(x+s-4,y-s//8,x+s+s//2,y+s//2,start=270,extent=180,outline="#9a7010",style="arc",width=max(3,s//4))
            # Liquid lines
            for ly_ in range(y-s//6,y+s-2,5):
                c.create_line(x-s+4,ly_,x+s-4,ly_,fill="#e8b840",width=1)
            # Rivets
            for ry_ in [y-s//6,y+s//3]:
                c.create_oval(x-s+2,ry_-1,x-s+6,ry_+3,fill="#9a7010",outline="")

        elif fid=="f_cactus":
            # Cactus with spines and flower
            c.create_rectangle(x-3,y-s,x+3,y+s,fill=col,outline="#1a5a10",width=k)
            c.create_rectangle(x-s+2,y-s//4,x-3,y+s//4,fill=col,outline="#1a5a10",width=k)
            c.create_rectangle(x+3,y-s//2,x+s-2,y+s//8,fill=col,outline="#1a5a10",width=k)
            # Spines
            for sx_,sy_ in [(-3,y-s+3),(-3,y-s//3),(3,y-s//2),(3,y+s//4),(-s+2,y-s//6),(s-2,y-s//3)]:
                c.create_line(x+sx_,sy_,x+sx_-4,sy_-3,fill="#88cc88",width=1)
                c.create_line(x+sx_,sy_,x+sx_-4,sy_+3,fill="#88cc88",width=1)
            # Flower
            for fa_ in range(0,360,72):
                c.create_oval(x-4+int(5*math.cos(math.radians(fa_))),
                              y-s-4+int(5*math.sin(math.radians(fa_))),
                              x+2+int(5*math.cos(math.radians(fa_))),
                              y-s+2+int(5*math.sin(math.radians(fa_))),fill="#ff88aa",outline="")
            c.create_oval(x-2,y-s-2,x+2,y-s+2,fill="#ffee00",outline="")

        elif fid=="f_cat":
            # Sleeping cat with fur detail
            c.create_oval(x-s+2,y+s//4,x+s-2,y+s,fill=col,outline="#555",width=k)  # body curl
            c.create_oval(x-s//2,y-s,x+s//2,y+s//4,fill=col,outline="#555",width=k)  # head
            # Ears
            c.create_polygon(x-s//2+2,y-s+4,x-s//2-5,y-s-8,x-s//2+8,y-s,fill=col,outline="#444")
            c.create_polygon(x-s//2+4,y-s+4,x-s//2-2,y-s-4,x-s//2+8,y-s,fill="#ff9999",outline="")
            c.create_polygon(x+s//2-2,y-s+4,x+s//2+5,y-s-8,x+s//2-8,y-s,fill=col,outline="#444")
            c.create_polygon(x+s//2-4,y-s+4,x+s//2+2,y-s-4,x+s//2-8,y-s,fill="#ff9999",outline="")
            # Eyes (closed)
            c.create_arc(x-s//4-3,y-s//4-2,x-s//4+3,y-s//4+2,start=0,extent=180,outline="#333",style="arc",width=k)
            c.create_arc(x+s//4-3,y-s//4-2,x+s//4+3,y-s//4+2,start=0,extent=180,outline="#333",style="arc",width=k)
            # Nose & whiskers
            c.create_oval(x-2,y-s//8,x+2,y-s//8+3,fill="#ff8899",outline="")
            for wx_,wy_ in [(-s,-s//8+1),(-s//2,-s//8-1),(s//2,-s//8-1),(s,-s//8+1)]:
                c.create_line(x+wx_//2,y+wy_,x+wx_,y+wy_+1,fill="#888",width=1)
            # Tail curl
            c.create_arc(x+s//4,y+s//4,x+s+s//3,y+s+s//3,start=90,extent=270,outline=col,style="arc",width=max(3,s//4))
            # Stripe marks
            for si_ in range(3):
                c.create_line(x-s+4,y+s//2+si_*3,x-s+10,y+s//2+si_*3+2,fill="#777",width=1)

        elif fid=="f_parrot":
            # Tropical parrot on perch
            c.create_line(x-s,y+s//2,x+s,y+s//2,fill="#7a5530",width=max(3,s//4))  # perch
            c.create_oval(x-s//3,y-s,x+s//3,y-s//3,fill="#22aa44",outline="#1a8a30",width=k)  # head
            c.create_oval(x-s//2,y-s//3-2,x+s//2,y+s//4,fill="#cc6600",outline="#885500",width=k)  # body
            c.create_polygon(x-s//2,y+s//8,x-s,y+s//2,x-s+4,y+s//2,fill="#ff4400",outline="")  # wing
            c.create_polygon(x+s//2,y+s//8,x+s,y+s//2,x+s-4,y+s//2,fill="#ffaa00",outline="")  # wing R
            c.create_polygon(x-5,y-s//3,x-s//2,y-s//8,x+5,y-s//3,fill="#ffcc00",outline="")  # beak
            c.create_oval(x-3,y-s//2-1,x+1,y-s//2+3,fill="#111",outline="")  # eye
            c.create_line(x-s//3,y+s//4,x-s//3-2,y+s//2,fill="#885500",width=2)  # claw
            c.create_line(x+s//3,y+s//4,x+s//3+2,y+s//2,fill="#885500",width=2)

        elif fid=="f_boot":
            # Worn leather boot with laces
            c.create_rectangle(x-s+2,y-s//2,x+s//4,y+s,fill=col,outline="#3a2008",width=k+1)
            c.create_rectangle(x-s+2,y+s//3,x+s,y+s,fill=col,outline="#3a2008",width=k+1)
            c.create_arc(x-s,y-s,x-s+10,y-s+10,start=90,extent=90,outline="#3a2008",style="arc",width=k)
            # Sole
            c.create_rectangle(x-s,y+s-1,x+s+2,y+s+3,fill="#222",outline="")
            # Lace holes
            for ly_ in range(y-s//4,y+s//4,6):
                c.create_oval(x-s//4-2,ly_,x-s//4+2,ly_+4,fill="#2a1400",outline="#888",width=1)
                c.create_oval(x+s//8-2,ly_,x+s//8+2,ly_+4,fill="#2a1400",outline="#888",width=1)
            # Laces
            for li_ in range(3):
                c.create_line(x-s//4,y-s//4+li_*6,x+s//8,y-s//4+li_*6,fill="#ccbbaa",width=1)
            # Scuff marks
            c.create_line(x-s+4,y+s//5,x-s+10,y+s//5+2,fill="#2a1400",width=1)

        elif fid=="f_star":
            # 5-pointed star with glow layers
            for layer in range(3):
                offs=layer*2; pts=[]
                for si_ in range(5):
                    a_=math.radians(si_*72-90); ai_=math.radians(si_*72+36-90)
                    pts+=[x+int((s-offs)*math.cos(a_)),y+int((s-offs)*math.sin(a_)),
                          x+int((s//2-offs)*math.cos(ai_)),y+int((s//2-offs)*math.sin(ai_))]
                fills=["#ffffee","#ffffaa",col][layer]
                c.create_polygon(pts,fill=fills,outline="#aa8800" if layer==0 else fills)
            c.create_oval(x-3,y-3,x+3,y+3,fill="#ffffff",outline="")
            # Sparkle lines
            for spi_ in range(4):
                sa_=math.radians(spi_*45+22)
                c.create_line(x,y,x+int((s+4)*math.cos(sa_)),y+int((s+4)*math.sin(sa_)),fill="#ffeeaa",width=1)

        elif fid=="f_mushroom":
            # Red mushroom with white spots and stem
            c.create_arc(x-s,y-s,x+s,y+6,start=0,extent=180,fill=col,outline="#881100",width=k+1)
            c.create_rectangle(x-s//2,y+4,x+s//2,y+s,fill="#e8e0d0",outline="#aaa",width=k)
            # Stem lines
            c.create_line(x-s//2+2,y+s//4,x-s//2+4,y+s,fill="#ccc",width=1)
            c.create_line(x+s//2-2,y+s//4,x+s//2-4,y+s,fill="#ccc",width=1)
            # Spots
            for mx_,my_ in [(-s//2+3,y-s//2),(-2,y-s+4),(s//2-5,y-s//2+4),(-s//3,y-s+10),(s//3,y-s//3)]:
                c.create_oval(x+mx_-4,my_-4,x+mx_+4,my_+4,fill="#ffffff",outline="#ddd",width=1)
            # Gill lines under cap
            for gx_ in range(x-s//2+2,x+s//2,5):
                c.create_line(gx_,y+2,gx_,y+6,fill="#cc9988",width=1)

        elif fid=="f_snake_sm":
            # Small garden snake coiled with scales
            for ri_ in range(3):
                r_=s-ri_*4
                c.create_oval(x-r_,y-r_//2,x+r_,y+r_//2,fill="",outline=col,width=max(3,4-ri_))
            # Scale texture (dots)
            for ssx_ in range(-s+4,s-3,5):
                for ssy_ in range(-4,4,4):
                    if (ssx_//5+ssy_//4)%2==0:
                        c.create_oval(x+ssx_,y+ssy_,x+ssx_+2,y+ssy_+2,fill="#336622",outline="")
            # Head
            c.create_oval(x+s-6,y-s//2-5,x+s+3,y-s//2+5,fill=col,outline="#336622",width=k)
            c.create_oval(x+s-3,y-s//2-2,x+s+1,y-s//2+1,fill="#cc0000",outline="")
            c.create_oval(x+s-2,y-s//2-3,x+s,y-s//2-1,fill="#111",outline="")

        elif fid=="f_frog":
            # Green frog with detail
            c.create_oval(x-s+2,y-s//4,x+s-2,y+s-2,fill=col,outline="#1a6a10",width=k)
            # Eyes on top
            c.create_oval(x-s+2,y-s,x-s+12,y-s+10,fill=col,outline="#1a6a10",width=k)
            c.create_oval(x+s-12,y-s,x+s-2,y-s+10,fill=col,outline="#1a6a10",width=k)
            c.create_oval(x-s+4,y-s+1,x-s+9,y-s+7,fill="#111",outline="")
            c.create_oval(x+s-9,y-s+1,x+s-4,y-s+7,fill="#111",outline="")
            c.create_oval(x-s+5,y-s+2,x-s+7,y-s+4,fill="#fff",outline="")
            c.create_oval(x+s-7,y-s+2,x+s-5,y-s+4,fill="#fff",outline="")
            # Belly
            c.create_oval(x-s//2,y-s//4+2,x+s//2,y+s-4,fill="#88cc44",outline="")
            # Nostrils
            c.create_oval(x-3,y-s//4+2,x-1,y-s//4+4,fill="#1a6a10",outline="")
            c.create_oval(x+1,y-s//4+2,x+3,y-s//4+4,fill="#1a6a10",outline="")
            # Back legs
            c.create_line(x-s+2,y+s//2,x-s-2,y+s+4,fill=col,width=max(3,k+1))
            c.create_line(x-s-2,y+s+4,x-s+4,y+s+8,fill=col,width=max(3,k+1))
            c.create_line(x+s-2,y+s//2,x+s+2,y+s+4,fill=col,width=max(3,k+1))
            c.create_line(x+s+2,y+s+4,x+s-4,y+s+8,fill=col,width=max(3,k+1))

        elif fid=="f_candle":
            # Detailed candle with melted wax drips
            c.create_rectangle(x-s//3,y,x+s//3,y+s,fill=col,outline="#aaa",width=k)
            # Wax drips
            for dx_,side in [(-s//3,True),(s//3,False)]:
                for di_ in range(0,s,8):
                    if (di_//8)%2==0:
                        c.create_oval(x+dx_-2,y+di_,x+dx_+2,y+di_+6,fill="#fffce8",outline="")
            # Candle texture lines
            for cy_ in range(y+4,y+s-2,4):
                c.create_line(x-s//3+2,cy_,x+s//3-2,cy_,fill="#e8d890",width=1)
            # Wick
            c.create_line(x,y-6,x,y+2,fill="#333",width=k)
            # Flame
            c.create_polygon(x-4,y-6,x,y-s,x+4,y-6,fill="#ff8800",outline="")
            c.create_polygon(x-2,y-6,x,y-s+4,x+2,y-6,fill="#ffee44",outline="")
            c.create_oval(x-1,y-7,x+1,y-5,fill="#ffffff",outline="")
            # Wax pool
            c.create_oval(x-s//3-2,y-3,x+s//3+2,y+3,fill="#fffce8",outline="#e8d890",width=1)

        elif fid=="f_anchor":
            # Ship anchor with chain
            thick=max(3,s//4)
            c.create_oval(x-s//3,y-s,x+s//3,y-s//3,fill="",outline=col,width=thick)
            c.create_line(x,y-s//3,x,y+s-4,fill=col,width=thick)
            c.create_line(x-s+2,y,x+s-2,y,fill=col,width=thick)
            c.create_arc(x-s+2,y+s//3,x,y+s+6,start=180,extent=90,outline=col,style="arc",width=thick)
            c.create_arc(x,y+s//3,x+s-2,y+s+6,start=270,extent=90,outline=col,style="arc",width=thick)
            # End knobs
            c.create_oval(x-s,y-2,x-s+6,y+4,fill=col,outline="")
            c.create_oval(x+s-6,y-2,x+s,y+4,fill=col,outline="")
            c.create_oval(x-3,y-s-2,x+3,y-s+4,fill=col,outline="")
            # Chain links
            for ci_ in range(3):
                cy_=y-s+ci_*6
                c.create_arc(x+s//4,cy_,x+s//4+5,cy_+4,start=0,extent=180,outline="#888",style="arc",width=1)
            # Shine
            c.create_line(x-s//4,y-s//2,x-s//4+2,y-s//2+s//3,fill="#aaaaaa",width=1)

        elif fid=="f_lantern":
            # Paper lantern with light glow
            c.create_oval(x-s+2,y-s//2,x+s-2,y+s-2,fill=col,outline="#882200",width=k+1)
            c.create_rectangle(x-3,y-s,x+3,y-s//2,fill="#aaa",outline="#777",width=k)
            c.create_oval(x-4,y-s-3,x+4,y-s+3,fill="#888",outline="#555",width=1)
            # Vertical ribs
            for rib_ in range(x-s+4,x+s-2,8):
                c.create_line(rib_,y-s//2,rib_,y+s-4,fill="#cc4400",width=1)
            # Inner glow
            c.create_oval(x-s//2,y-s//4,x+s//2,y+s//2,fill="#ff8800",outline="")
            c.create_oval(x-s//4,y-s//8,x+s//4,y+s//4,fill="#ffcc44",outline="")
            # Tassel
            c.create_line(x,y+s-2,x,y+s+6,fill="#cc2200",width=k)
            c.create_oval(x-3,y+s+4,x+3,y+s+10,fill="#cc2200",outline="")

        elif fid=="f_clover":
            # 4-leaf clover with stem and veins
            for ca_ in [0,90,180,270]:
                cx_=x+int(s//3*math.cos(math.radians(ca_))); cy_=y+int(s//3*math.sin(math.radians(ca_)))
                c.create_oval(cx_-s//2,cy_-s//2,cx_+s//2,cy_+s//2,fill=col,outline="#1a6a10",width=k)
                c.create_line(x,y,cx_,cy_,fill="#2a7a20",width=k)
                c.create_line(cx_,cy_,cx_+int(s//4*math.cos(math.radians(ca_+30))),cy_+int(s//4*math.sin(math.radians(ca_+30))),fill="#3a8a30",width=1)
            c.create_oval(x-3,y-3,x+3,y+3,fill="#2a7a20",outline="")
            c.create_line(x,y,x,y+s,fill="#2a7a20",width=k+1)
            c.create_line(x,y+s-4,x-4,y+s+4,fill="#2a7a20",width=k)
            c.create_line(x,y+s-4,x+4,y+s+4,fill="#2a7a20",width=k)

        elif fid=="f_barrel":
            # Oak barrel with metal bands and bung
            c.create_rectangle(x-s+2,y-s//2,x+s-2,y+s,fill=col,outline="#3a1800",width=k+1)
            # Curved sides
            for yy_ in range(y-s//2,y+s,4):
                shade="#8B3500" if (yy_//8)%2==0 else "#6a2800"
                c.create_line(x-s+2,yy_,x+s-2,yy_,fill=shade,width=2)
            # Metal bands
            for by_ in [y-s//4,y+s//4]:
                c.create_rectangle(x-s,by_,x+s,by_+4,fill="#555",outline="#333",width=1)
                c.create_oval(x-s,by_,x-s+5,by_+4,fill="#777",outline="")
                c.create_oval(x+s-5,by_,x+s,by_+4,fill="#777",outline="")
            # Top
            c.create_oval(x-s+2,y-s//2-4,x+s-2,y-s//2+6,fill="#7a4020",outline="#3a1800",width=k)
            # Bung hole
            c.create_oval(x-3,y-3,x+3,y+3,fill="#2a0800",outline="#553010",width=1)

        elif fid=="f_spade":
            # Spade playing suit with handle
            # Spade head
            pts_=[x,y-s,x-s+2,y,x-s//3,y+2,x-s//3,y+s,x+s//3,y+s,x+s//3,y+2,x+s-2,y]
            c.create_polygon(pts_,fill=col,outline="#222",width=k)
            # Inner shine
            c.create_polygon(x,y-s+4,x-s+6,y-2,x-s//3+2,y+2,x+s//3-2,y+2,x+s-6,y-2,fill="#556666",outline="")
            c.create_line(x,y-s+2,x,y-s//2,fill="#778888",width=k)

        elif fid=="f_gem_sm":
            # Cut gemstone with facets
            c.create_polygon(x,y-s,x-s+2,y-2,x-s//2,y+s-2,x+s//2,y+s-2,x+s-2,y-2,fill=col,outline="#3366aa",width=k)
            # Facet lines
            c.create_line(x,y-s,x-s+2,y-2,fill="#88aaff",width=1)
            c.create_line(x,y-s,x+s-2,y-2,fill="#88aaff",width=1)
            c.create_line(x-s+2,y-2,x+s-2,y-2,fill="#88aaff",width=1)
            c.create_line(x-s+2,y-2,x-s//2,y+s-2,fill="#4466cc",width=1)
            c.create_line(x+s-2,y-2,x+s//2,y+s-2,fill="#4466cc",width=1)
            c.create_line(x,y-s,x,y+s-2,fill="#aaccff",width=1)
            c.create_oval(x-3,y-s+2,x+3,y-s+8,fill="#ccddff",outline="")

        # ── HORSES ───────────────────────────────────────
        elif "horse" in fid:
            variant=fid.split("_")[-1]
            mane_col={"tb":"#ff6600","nm":"#8888ff","ls":"#ffee88","im":"#aaaaaa","sb":"#cc66ff","gold":"#ffee00"}.get(variant,"#888")
            eye_col ={"tb":"#ff4400","nm":"#4488ff","ls":"#ffcc00","im":"#66cc44","sb":"#aa44ff","gold":"#ffdd00"}.get(variant,"#111")
            # Body
            c.create_oval(x-s+4,y-s+10,x+s-2,y+8,fill=col,outline="#333",width=k)
            # Neck
            c.create_polygon(x-s+6,y-s+8,x-s+4,y-s-2,x-s+14,y-s,x-s+12,y-s+10,fill=col,outline="#333",width=1)
            # Head
            c.create_oval(x-s+2,y-s-4,x-s+16,y-s+8,fill=col,outline="#333",width=k)
            # Nose/muzzle
            c.create_oval(x-s+2,y-s+2,x-s+10,y-s+10,fill=col,outline="#555",width=1)
            c.create_oval(x-s+3,y-s+5,x-s+6,y-s+8,fill="#443322",outline="")
            # Eye
            c.create_oval(x-s+9,y-s-1,x-s+14,y-s+4,fill=eye_col,outline="#111",width=1)
            c.create_oval(x-s+10,y-s,x-s+13,y-s+3,fill="#111",outline="")
            # Ear
            c.create_polygon(x-s+10,y-s-4,x-s+7,y-s-10,x-s+14,y-s-4,fill=col,outline="#333")
            c.create_polygon(x-s+10,y-s-3,x-s+9,y-s-7,x-s+13,y-s-3,fill=mane_col,outline="")
            # Mane
            for mi_ in range(4):
                c.create_line(x-s+12-mi_*2,y-s-2,x-s+8-mi_*2,y-s+8,fill=mane_col,width=max(2,k+1))
            # Legs
            for li_,lx_ in enumerate([x-s+6,x-s+10,x+s-12,x+s-8]):
                c.create_line(lx_,y+6,lx_+(1 if li_%2 else -1),y+s,fill=col,width=max(3,k+1))
                c.create_oval(lx_-2,y+s-2,lx_+4,y+s+4,fill="#333",outline="")  # hoof
            # Tail
            c.create_line(x+s-4,y-s+12,x+s+4,y-s,fill=mane_col,width=max(3,k+1))
            c.create_line(x+s+4,y-s,x+s+6,y-s+8,fill=mane_col,width=max(2,k))
            if variant=="gold":
                c.create_text(x+s-2,y-s+2,text="★",fill=GOLD,font=tkfont.Font(size=max(4,s//2)))
            elif variant=="nm":  # Nightmare — glowing eyes
                c.create_oval(x-s+8,y-s-2,x-s+15,y-s+5,fill="",outline="#6688ff",width=1)

        elif fid in ("f_casino_sm","f_casino_big"):
            # Mini casino facade
            sc=1.5 if "big" in fid else 1
            bw=int(s*sc)
            c.create_rectangle(x-bw+2,y-2,x+bw-2,y+s,fill=col,outline="#3d0a0a",width=k)
            c.create_polygon(x-bw+2,y-2,x,y-s+2,x+bw-2,y-2,fill="#cc7700",outline="#885500",width=k)
            c.create_rectangle(x-4,y+s//4,x+4,y+s-2,fill="#0a0028",outline="#6600cc",width=1)
            for wx_ in [-bw+6,-bw+12,bw-12,bw-6]:
                c.create_rectangle(x+wx_,y+2,x+wx_+5,y+s//2,fill="#1a0028",outline="#880000",width=1)
            c.create_text(x,y-s//2,text="★",fill=GOLD,font=tkfont.Font(size=max(4,s//2),weight="bold"))
            if "big" in fid:
                for gix_ in range(-bw+4,bw-3,6):
                    c.create_oval(x+gix_,y-5,x+gix_+4,y-1,fill="#ffcc00",outline="")

        elif fid=="f_stable_sm":
            c.create_rectangle(x-s+2,y,x+s-2,y+s,fill=col,outline="#3a1800",width=k)
            c.create_polygon(x-s+2,y,x,y-s+4,x+s-2,y,fill="#5a2800",outline="#3a1800",width=k)
            c.create_rectangle(x-5,y+s//3,x+5,y+s,fill="#2a1000",outline="")
            c.create_line(x,y+s//3,x,y+s,fill="#3a1800",width=1)
            # Hay bale
            c.create_rectangle(x-s+4,y+s//2,x-s+10,y+s-2,fill="#cc9900",outline="#886600",width=1)
            for hy_ in range(y+s//2,y+s-2,3):
                c.create_line(x-s+4,hy_,x-s+10,hy_,fill="#aa8800",width=1)
            # Horseshoe on wall
            c.create_arc(x+s-12,y+4,x+s-4,y+12,start=0,extent=180,outline="#888",style="arc",width=k)

        elif fid=="f_bank_sm":
            c.create_rectangle(x-s+2,y-2,x+s-2,y+s,fill=col,outline="#1a2a4a",width=k)
            for bpx_ in [x-s+5,x+s-9]:
                c.create_rectangle(bpx_,y-2,bpx_+4,y+s,fill="#d0c8b0",outline="#b0a880",width=1)
            c.create_polygon(x-s+2,y-2,x,y-s,x+s-2,y-2,fill="#2a3a6a",outline="#1a2a4a",width=k)
            # Vault
            c.create_oval(x-5,y+s//3,x+5,y+s//3+10,fill="#777",outline="#444",width=k)
            c.create_line(x,y+s//3+5,x+4,y+s//3+2,fill="#555",width=k)
            # Windows
            for wbx_ in [x-s//2,x+s//4]:
                c.create_rectangle(wbx_-3,y+2,wbx_+3,y+s//3-2,fill="#c8e0f8",outline="#888",width=1)

        elif fid=="f_shop_sm":
            c.create_rectangle(x-s+2,y-2,x+s-2,y+s,fill=col,outline="#1a5a3a",width=k)
            c.create_rectangle(x-s+2,y-2,x+s-2,y+s//3,fill="#2a7a5a",outline="")
            c.create_text(x,y+s//8,text="SHOP",fill="#fffef0",font=tkfont.Font(family="Courier New",size=max(4,s//3)))
            c.create_rectangle(x-4,y+s//3,x+4,y+s,fill="#2a1800",outline="")
            c.create_rectangle(x-s+4,y+s//2,x-2,y+s-2,fill="#f8e0a0",outline="#c8a060",width=1)

        elif fid=="f_snake_co":
            # Rearing cobra with hood and crown
            c.create_line(x,y+s,x,y-s//3,fill=col,width=max(4,s//3))  # body
            # Scale texture
            for si_ in range(y-s//3,y+s,6):
                c.create_oval(x-2,si_,x+2,si_+3,fill="#aa8800",outline="")
            # Hood
            c.create_oval(x-s//2,y-s,x+s//2,y-s//3,fill=col,outline="#886600",width=k+1)
            # Hood markings
            c.create_oval(x-s//3,y-s+4,x+s//3,y-s//2,fill="#cc9900",outline="")
            # Head
            c.create_oval(x-4,y-s//2-3,x+4,y-s//2+5,fill=col,outline="#886600",width=k)
            # Eyes
            c.create_oval(x-4,y-s//2-1,x-1,y-s//2+2,fill="#ff6600",outline="#111",width=1)
            c.create_oval(x+1,y-s//2-1,x+4,y-s//2+2,fill="#ff6600",outline="#111",width=1)
            c.create_oval(x-3,y-s//2,x-2,y-s//2+1,fill="#111",outline="")
            c.create_oval(x+2,y-s//2,x+3,y-s//2+1,fill="#111",outline="")
            # Forked tongue
            c.create_line(x,y-s//2+5,x-3,y-s//2+10,fill="#cc0000",width=1)
            c.create_line(x,y-s//2+5,x+3,y-s//2+10,fill="#cc0000",width=1)

        elif fid=="f_snake_py":
            # Python coiled on branch
            c.create_line(x-s,y+s//3,x+s,y+s//3,fill="#7a5530",width=max(4,s//3))  # branch
            # Coils
            for ri_ in range(3):
                r_=s-ri_*5
                c.create_oval(x-r_,y-r_//2,x+r_,y+r_//2,fill="",outline=col,width=max(3,4-ri_))
            # Scale dots
            for ssx_,ssy_ in [(0,-s//2),(s//2,0),(-s//2,0),(0,s//4)]:
                c.create_oval(x+ssx_-3,y+ssy_-2,x+ssx_+3,y+ssy_+2,fill="#226622",outline="")
            # Head
            c.create_oval(x+s-7,y-3,x+s+3,y+7,fill=col,outline="#226622",width=k)
            c.create_oval(x+s-4,y-1,x+s,y+2,fill="#111",outline="")
            c.create_oval(x+s-3,y,x+s-1,y+2,fill="#ffcc00",outline="")
            c.create_line(x+s+3,y+3,x+s+6,y,fill="#cc0000",width=1)
            c.create_line(x+s+3,y+3,x+s+6,y+6,fill="#cc0000",width=1)

        elif fid=="f_wolf":
            # Howling silver wolf
            c.create_oval(x-s+2,y-s//4,x+s-2,y+s,fill=col,outline="#777",width=k)  # body
            c.create_oval(x-s//2,y-s,x+s//2,y-s//4+4,fill=col,outline="#777",width=k)  # head
            # Pointed ears
            c.create_polygon(x-s//2+2,y-s+2,x-s//2-6,y-s-10,x-s//2+10,y-s+2,fill=col,outline="#555")
            c.create_polygon(x-s//2+4,y-s+2,x-s//2-2,y-s-5,x-s//2+9,y-s+2,fill="#ff8888",outline="")
            c.create_polygon(x+s//2-2,y-s+2,x+s//2+6,y-s-10,x+s//2-10,y-s+2,fill=col,outline="#555")
            c.create_polygon(x+s//2-4,y-s+2,x+s//2+2,y-s-5,x+s//2-9,y-s+2,fill="#ff8888",outline="")
            # Eyes (glowing)
            c.create_oval(x-s//4-3,y-s//2-1,x-s//4+3,y-s//2+5,fill="#ffee00",outline="#aaa",width=1)
            c.create_oval(x+s//4-3,y-s//2-1,x+s//4+3,y-s//2+5,fill="#ffee00",outline="#aaa",width=1)
            c.create_oval(x-s//4-1,y-s//2+1,x-s//4+1,y-s//2+3,fill="#111",outline="")
            c.create_oval(x+s//4-1,y-s//2+1,x+s//4+1,y-s//2+3,fill="#111",outline="")
            # Snout
            c.create_oval(x-s//4,y-s//4-2,x+s//4,y-s//4+6,fill=col,outline="#666",width=1)
            c.create_oval(x-3,y-s//4,x+3,y-s//4+4,fill="#222",outline="")
            # Howling mouth open
            c.create_arc(x-s//4,y-s//4+2,x+s//4,y-s//4+10,start=200,extent=140,outline="#111",style="arc",width=k)
            # Fur marks
            for fy_ in range(3): c.create_line(x-s+4,y+fy_*8,x-s+8,y+fy_*8+4,fill="#aaaaaa",width=1)
            # Tail raised
            c.create_arc(x+s//2,y,x+s+s//2,y+s,start=90,extent=200,outline=col,style="arc",width=max(3,k+1))

        elif fid=="f_owl":
            # Wise owl with spectacles
            c.create_oval(x-s+2,y-s//4,x+s-2,y+s,fill=col,outline="#6a5030",width=k)  # body
            c.create_oval(x-s//2,y-s,x+s//2,y-s//4,fill=col,outline="#6a5030",width=k)  # head
            # Feather tufts
            c.create_polygon(x-s//2+2,y-s+2,x-s//2-4,y-s-8,x-s//2+8,y-s,fill=col,outline="#554020")
            c.create_polygon(x+s//2-2,y-s+2,x+s//2+4,y-s-8,x+s//2-8,y-s,fill=col,outline="#554020")
            # Spectacles
            c.create_oval(x-s//2+2,y-s+4,x-s//2+s//3+2,y-s+4+s//3,fill="",outline="#cc8800",width=k)
            c.create_oval(x+s//2-s//3-2,y-s+4,x+s//2-2,y-s+4+s//3,fill="",outline="#cc8800",width=k)
            c.create_line(x-s//2+s//3+2,y-s+4+s//6,x+s//2-s//3-2,y-s+4+s//6,fill="#cc8800",width=k)
            # Eyes behind specs
            c.create_oval(x-s//2+4,y-s+6,x-s//2+s//3,y-s+4+s//3-2,fill="#ffee00",outline="")
            c.create_oval(x+s//2-s//3,y-s+6,x+s//2-4,y-s+4+s//3-2,fill="#ffee00",outline="")
            c.create_oval(x-s//4,y-s+8,x-s//4+3,y-s+11,fill="#111",outline="")
            c.create_oval(x+s//4-2,y-s+8,x+s//4+1,y-s+11,fill="#111",outline="")
            # Beak
            c.create_polygon(x-3,y-s//2+2,x,y-s//2-4,x+3,y-s//2+2,fill="#cc8800",outline="#886600")
            # Wing markings
            for wi_ in range(3):
                c.create_arc(x-s+3,y+wi_*8,x+s-3,y+wi_*8+8,start=0,extent=180,outline="#887755",style="arc",width=1)
            # Feet
            c.create_line(x-s//3,y+s,x-s//3-3,y+s+5,fill="#cc8800",width=k)
            c.create_line(x-s//3,y+s,x-s//3+3,y+s+5,fill="#cc8800",width=k)
            c.create_line(x+s//3,y+s,x+s//3-3,y+s+5,fill="#cc8800",width=k)
            c.create_line(x+s//3,y+s,x+s//3+3,y+s+5,fill="#cc8800",width=k)
            ...
            c.create_oval(x-s,y-s,x+s,y+s,fill=col,outline="#6688aa",width=k+1)
            c.create_oval(x-s+2,y-s+2,x+s-2,y+s-2,fill="#aaccff",outline="")
            # Swirling mist arcs
            for mi_ in range(3):
                c.create_arc(x-s+4+mi_*4,y-s+4+mi_*4,x+s-4-mi_*4,y+s-4-mi_*4,start=mi_*60,extent=200,outline="#ddeeff",style="arc",width=max(2,k))
            # Inner glow
            c.create_oval(x-s//3,y-s//3,x+s//3,y+s//3,fill="#cce8ff",outline="")
            c.create_oval(x-s//5,y-s//5,x+s//5,y+s//5,fill="#ffffff",outline="")
            # Highlight top-left
            c.create_arc(x-s+3,y-s+3,x,y,start=30,extent=60,outline="#ffffff",style="arc",width=k+1)
            # Stand
            c.create_rectangle(x-s//2,y+s-2,x+s//2,y+s+4,fill="#4a3820",outline="#2a1800",width=1)
            c.create_rectangle(x-s//3,y+s+3,x+s//3,y+s+6,fill="#3a2810",outline="#2a1800",width=1)

        elif fid=="f_jester":
            # Jester mid-somersault
            c.create_oval(x-s//2,y-s,x+s//2,y-s//3,fill=col,outline="#880044",width=k)  # head
            # Jester hat (two-pointed)
            c.create_polygon(x-s//2,y-s,x-s-4,y-s-s//2,x-2,y-s,fill="#cc0066",outline="#880044")
            c.create_polygon(x+s//2,y-s,x+s+4,y-s-s//2,x+2,y-s,fill="#ffcc00",outline="#886600")
            c.create_oval(x-s-6,y-s-s//2-4,x-s-1,y-s-s//2+1,fill="#ffcc00",outline="")  # bell L
            c.create_oval(x+s+1,y-s-s//2-4,x+s+6,y-s-s//2+1,fill="#cc0066",outline="")  # bell R
            # Body (torso in spin)
            c.create_oval(x-s+2,y-s//3,x+s-2,y+s//2,fill="#ddaa00",outline="#885500",width=k)
            # Diamond pattern
            for di_x,di_y in [(x-s//2,y-s//4+s//8),(x,y-s//3+s//6),(x+s//2,y-s//4+s//8)]:
                c.create_polygon(di_x,di_y-4,di_x-4,di_y,di_x,di_y+4,di_x+4,di_y,fill="#cc0066" if di_x==x else "#3388ff",outline="")
            c.create_rectangle(x-s+4,y+s//2,x+s-4,y+s,fill="#cc0066",outline="#880044",width=k)
            # Arms in spin
            c.create_line(x-s+2,y-s//8,x-s-4,y-s//3,fill=col,width=max(3,k+1))
            c.create_line(x+s-2,y-s//8,x+s+4,y-s//3,fill="#ddaa00",width=max(3,k+1))
            # Legs crossed in spin
            c.create_line(x-s//4,y+s//2,x-s//2,y+s,fill=col,width=max(3,k+1))
            c.create_line(x+s//4,y+s//2,x+s//2,y+s,fill="#ddaa00",width=max(3,k+1))

        # ── RARE fighters ────────────────────────────────
        elif fid in ("f_ff_razr","f_ff_slck","f_ff_blze","f_ff_iron"):
            stance={"f_ff_razr":"fire","f_ff_slck":"ice","f_ff_blze":"kick","f_ff_iron":"fist"}[fid]
            skin={"f_ff_razr":"#f08060","f_ff_slck":"#80a8f0","f_ff_blze":"#f0a060","f_ff_iron":"#80c880"}[fid]
            # Detailed fighter sprite
            c.create_oval(x-s//2,y-s,x+s//2,y-s//3,fill=skin,outline="#333",width=k)  # head
            # Hair
            hair_col={"fire":"#ff4400","ice":"#88aaff","kick":"#cc6600","fist":"#226622"}[stance]
            for hi_ in range(x-s//3,x+s//3+1,4):
                c.create_line(hi_,y-s,hi_,y-s-5-((hi_-x)//4)**2,fill=hair_col,width=2)
            c.create_rectangle(x-s//2+2,y-s//3,x+s//2-2,y+s//4,fill=col,outline="#333",width=k)  # torso
            # Belt
            c.create_rectangle(x-s//2+2,y+s//8,x+s//2-2,y+s//4,fill="#333",outline="#111",width=1)
            c.create_rectangle(x-3,y+s//8+1,x+3,y+s//4-1,fill="#cc8800",outline="#886600",width=1)
            # Legs
            c.create_rectangle(x-s//2+2,y+s//4,x-2,y+s,fill=col,outline="#333",width=k)
            c.create_rectangle(x+2,y+s//4,x+s//2-2,y+s,fill=col,outline="#333",width=k)
            # Boots
            c.create_rectangle(x-s//2+1,y+s-3,x-1,y+s+3,fill="#222",outline="#111",width=1)
            c.create_rectangle(x+1,y+s-3,x+s//2-1,y+s+3,fill="#222",outline="#111",width=1)
            if stance=="fire":
                # Fire stance: arm forward shooting fireball
                c.create_line(x+s//2-2,y-s//4,x+s+4,y-s//4-4,fill=col,width=max(3,k+1))
                for fi_ in range(3):
                    c.create_oval(x+s+fi_*4,y-s//4-4-fi_*2,x+s+fi_*4+6,y-s//4+2-fi_*2,fill=["#ff6600","#ff8800","#ffcc00"][fi_],outline="")
                c.create_line(x-s//2+2,y-s//4,x-s+2,y,fill=col,width=max(3,k+1))  # guard
            elif stance=="ice":
                c.create_line(x-s//2+2,y-s//4,x-s-2,y-s,fill=col,width=max(3,k+1))
                c.create_polygon(x-s-4,y-s-2,x-s-8,y-s-6,x-s,y-s+4,fill="#88ddff",outline="#44aacc")
                c.create_line(x+s//2-2,y-s//4,x+s+2,y-s//8,fill=col,width=max(3,k+1))
            elif stance=="kick":
                c.create_line(x-s//2+2,y-s//4,x-s+2,y-s//3,fill=col,width=max(3,k+1))
                c.create_line(x+2,y+s//4,x+s+2,y-s//8,fill=col,width=max(3,k+1))  # kick leg
                c.create_oval(x+s,y-s//8-5,x+s+10,y-s//8+5,fill="#ff8800",outline="")
            else:  # fist
                c.create_line(x-s//2+2,y-s//4,x-s-4,y-s//8,fill=col,width=max(3,k+1))
                c.create_rectangle(x-s-8,y-s//8-4,x-s,y-s//8+4,fill="#888",outline="#555",width=k)
                c.create_line(x+s//2-2,y-s//4,x+s+2,y-s//3,fill=col,width=max(3,k+1))

        elif fid=="f_arena_sm":
            # Mini arena — colosseum shape
            c.create_oval(x-s,y-s//4,x+s,y+s,fill=col,outline="#880000",width=k+1)
            c.create_oval(x-s+4,y,x+s-4,y+s-2,fill="#1a0000",outline="")
            # Archways
            for ai_ in range(6):
                ax_=x+int((s-3)*math.cos(math.radians(ai_*60))); ay_=y+s//2+int((s//2-2)*math.sin(math.radians(ai_*60)))
                c.create_rectangle(ax_-3,ay_-s//3,ax_+3,ay_,fill="#330000",outline=col,width=1)
                c.create_arc(ax_-3,ay_-s//3-4,ax_+3,ay_-s//3+2,start=0,extent=180,fill="#330000",outline=col)
            # Banner
            c.create_rectangle(x-s//3,y-s//4-4,x+s//3,y-s//4+4,fill="#cc0000",outline="#880000",width=1)
            c.create_text(x,y-s//4,text="!",fill=GOLD,font=tkfont.Font(size=max(4,s//2),weight="bold"))

        elif fid=="f_hotel_sm":
            # Mini grand hotel — 3 floors
            c.create_rectangle(x-s+2,y-s//3,x+s-2,y+s,fill=col,outline="#8B6914",width=k)
            for fl_ in range(3):
                fy_=y-s//3+fl_*(s//2+2)
                c.create_line(x-s+2,fy_,x+s-2,fy_,fill="#8B6914",width=k)
                # Windows
                for wx_ in [x-s//2,x+s//4]:
                    c.create_rectangle(wx_-3,fy_+4,wx_+3,fy_+s//3-2,fill="#c8e0f8",outline="#888",width=1)
            c.create_polygon(x-s+2,y-s//3,x,y-s,x+s-2,y-s//3,fill="#8B6914",outline="#6a4010",width=k)
            c.create_text(x,y+s-4,text="H",fill=GOLD,font=tkfont.Font(size=max(4,s//2),weight="bold"))

        elif fid=="f_den_sm":
            c.create_rectangle(x-s+4,y-2,x+s-4,y+s,fill=col,outline="#330033",width=k)
            c.create_polygon(x-s+4,y-2,x,y-s+4,x+s-4,y-2,fill="#550055",outline="#330033",width=k)
            c.create_rectangle(x-4,y+s//3,x+4,y+s,fill="#110011",outline="")
            # Neon sign
            c.create_rectangle(x-s+6,y+2,x+s-6,y+s//3,fill="#1a0028",outline="#aa00aa",width=1)
            c.create_text(x,y+s//6,text="DEN",fill="#cc00cc",font=tkfont.Font(family="Courier New",size=max(4,s//3)))
            c.create_oval(x-s+8,y+s//2,x-s+14,y+s//2+6,fill="#cc00cc",outline="")

        elif fid=="f_vip_sm":
            c.create_rectangle(x-s+4,y-2,x+s-4,y+s,fill=col,outline="#4a0d6a",width=k)
            c.create_polygon(x-s+4,y-2,x,y-s+4,x+s-4,y-2,fill="#6a2090",outline="#4a0d6a",width=k)
            c.create_text(x,y+s//2,text="VIP",fill=GOLD,font=tkfont.Font(size=max(4,s//2),weight="bold"))
            c.create_rectangle(x-4,y+2,x+4,y+s//3,fill="#2a0040",outline="#cc88ff",width=1)
            for vi_ in [x-s+6,x+s-10]:
                c.create_oval(vi_,y+2,vi_+6,y+8,fill="#ffcc00",outline="")

        elif fid=="f_dragon":
            # Baby dragon with scales and flame
            c.create_polygon(x,y-s,x-s+2,y+s//2,x,y,x+s-2,y+s//2,fill=col,outline="#550000",width=k)  # body
            # Wings
            c.create_polygon(x,y-s,x-s-4,y-s//2,x-s//2,y,fill=col,outline="#880000")
            c.create_polygon(x,y-s,x+s+4,y-s//2,x+s//2,y,fill=col,outline="#880000")
            # Wing membrane lines
            c.create_line(x-s,y-s//2,x-s//2,y,fill="#aa2200",width=1)
            c.create_line(x+s,y-s//2,x+s//2,y,fill="#aa2200",width=1)
            # Head
            c.create_oval(x-s//3,y-s,x+s//3,y-s//2,fill=col,outline="#550000",width=k)
            # Snout
            c.create_rectangle(x-s//4,y-s//2+2,x+s//4,y-s//2+8,fill=col,outline="#550000",width=1)
            # Eyes
            c.create_oval(x-s//4,y-s+2,x-s//4+4,y-s+6,fill="#ffaa00",outline="#111",width=1)
            c.create_oval(x+s//4-4,y-s+2,x+s//4,y-s+6,fill="#ffaa00",outline="#111",width=1)
            # Flame puffs
            for fi_ in range(3):
                c.create_oval(x+s//3+fi_*5,y-s//2+fi_*2-2,x+s//3+fi_*5+6,y-s//2+fi_*2+4,fill=["#ff4400","#ff8800","#ffcc00"][fi_],outline="")
            # Tail spike
            c.create_polygon(x+s-2,y+s//2,x+s+6,y+s//2-4,x+s+4,y+s//2+4,fill=col,outline="#550000")

        elif fid=="f_mermaid":
            # Mermaid on rock
            # Rock
            c.create_oval(x-s+2,y+s//3,x+s-2,y+s,fill="#888",outline="#555",width=k)
            # Tail
            c.create_polygon(x-s+4,y+s//4,x-s//2,y+s//3,x+s//2,y+s//3,x+s-4,y+s//4,
                             x+s//2,y-s//3,x-s//2,y-s//3,fill=col,outline="#1a7a8a",width=k)
            # Scales on tail
            for si_y in range(y-s//3,y+s//4,6):
                for si_x in range(x-s//3,x+s//3,8):
                    c.create_arc(si_x,si_y,si_x+7,si_y+5,start=0,extent=180,outline="#117a8a",style="arc",width=1)
            # Fin
            c.create_polygon(x-s+4,y+s//4,x-s-4,y-s//8,x-s//3,y-s//4,fill=col,outline="#1a7a8a")
            # Body
            c.create_oval(x-s//2,y-s,x+s//2,y-s//3,fill="#f0c8a0",outline="#c0a080",width=k)
            # Hair
            for hi_ in range(x-s//2,x+s//2,4):
                c.create_line(hi_,y-s,hi_+1,y-s+s//4,fill="#cc6688",width=2)
            # Shell top
            for sh_ in [x-s//3,x+s//4-6]:
                c.create_arc(sh_,y-s//2-2,sh_+8,y-s//2+8,start=0,extent=180,fill="#ffcc44",outline="#cc8800",width=1)
            # Arms
            c.create_line(x-s//2,y-s//2,x-s,y-s//8,fill="#f0c8a0",width=max(3,k))
            c.create_line(x+s//2,y-s//2,x+s,y-s//8,fill="#f0c8a0",width=max(3,k))

        # ── EPIC ────────────────────────────────────────
        elif fid=="f_snake_ki":
            # King cobra with jewelled crown
            c.create_line(x,y+s,x,y-s//3,fill=col,width=max(4,s//3))
            for si_ in range(y-s//3,y+s,7):
                c.create_oval(x-2,si_,x+2,si_+4,fill="#886600",outline="")
            c.create_oval(x-s//2,y-s,x+s//2,y-s//3,fill=col,outline="#886600",width=k+1)
            c.create_oval(x-s//3,y-s+6,x+s//3,y-s//2,fill="#ddaa00",outline="")
            c.create_oval(x-4,y-s//2-4,x+4,y-s//2+6,fill=col,outline="#886600",width=k)
            c.create_oval(x-3,y-s//2-1,x,y-s//2+2,fill="#ff4400",outline="")
            c.create_oval(x+1,y-s//2-1,x+4,y-s//2+2,fill="#ff4400",outline="")
            # Crown
            crown_pts=[x-s//2+2,y-s,x-s//2+2,y-s-8,x-s//4,y-s-4,x,y-s-10,x+s//4,y-s-4,x+s//2-2,y-s-8,x+s//2-2,y-s]
            c.create_polygon(crown_pts,fill=GOLD,outline="#886600",width=k)
            for jx_,jc_ in [(x-s//3,y-s-8),(x,y-s-9),(x+s//3,y-s-8)]:
                c.create_oval(jx_-2,jc_-2,jx_+2,jc_+2,fill=random.choice(["#ff2244","#2244ff","#22ff44"]),outline="")

        elif fid=="f_ff_champ":
            # Championship trophy — detailed
            c.create_rectangle(x-s//4,y-s,x+s//4,y-s//3,fill="#c8a000",outline=GOLD,width=k)  # stem
            c.create_oval(x-s+4,y-s//3,x+s-4,y+s//3,fill="#c8a000",outline=GOLD,width=k)  # cup
            c.create_arc(x-s+4,y-s//3,x-s//2,y+s//3,start=270,extent=180,outline=GOLD,style="arc",width=max(3,k+1))
            c.create_arc(x+s//2,y-s//3,x+s-4,y+s//3,start=90,extent=180,outline=GOLD,style="arc",width=max(3,k+1))
            c.create_rectangle(x-s//2,y+s//3,x+s//2,y+s-2,fill="#c8a000",outline=GOLD,width=k)  # base
            c.create_rectangle(x-s+2,y+s-4,x+s-2,y+s+2,fill="#c8a000",outline=GOLD,width=k)  # plinth
            c.create_text(x,y,text="★",fill=GOLD,font=tkfont.Font(size=max(6,s),weight="bold"))
            # Engraving
            c.create_text(x,y+s//2+2,text="FF",fill="#886600",font=tkfont.Font(size=max(4,s//3)))
            for sh_ in range(3):
                c.create_line(x-s//2+sh_*2,y-s//3+4,x-s//2+sh_*2,y+s//3-4,fill="#ffdd88",width=1)

        elif fid=="f_phoenix":
            # Phoenix in flight — wings spread
            c.create_polygon(x,y-s,x-s+2,y+s//2,x,y,x+s-2,y+s//2,fill=col,outline="#cc4400",width=k)
            # Wing detail
            for wi_ in range(4):
                wa_=math.radians(wi_*20+140); wb_=math.radians(wi_*20+150)
                c.create_polygon(x,y-s//2,x+int((s+8)*math.cos(wa_)),y-s//2+int((s+8)*math.sin(wa_)),
                                 x+int((s+4)*math.cos(wb_)),y-s//2+int((s+4)*math.sin(wb_)),fill="#ff4400",outline="")
                wa2_=math.radians(wi_*20+30); wb2_=math.radians(wi_*20+20)
                c.create_polygon(x,y-s//2,x+int((s+8)*math.cos(wa2_)),y-s//2+int((s+8)*math.sin(wa2_)),
                                 x+int((s+4)*math.cos(wb2_)),y-s//2+int((s+4)*math.sin(wb2_)),fill="#ff6600",outline="")
            # Flame feathers trailing
            for fi_ in range(5):
                fa_=math.radians(fi_*25+100)
                c.create_line(x,y,x+int((s+6+fi_*2)*math.cos(fa_)),y+int((s+6+fi_*2)*math.sin(fa_)),
                              fill=["#ff2200","#ff4400","#ff6600","#ff8800","#ffcc00"][fi_],width=max(2,k))
            c.create_oval(x-s//3,y-s+2,x+s//3,y-s//2,fill=col,outline="#cc4400",width=k)  # head
            c.create_oval(x-3,y-s+4,x+3,y-s+10,fill="#ffee00",outline="#cc8800",width=k)  # crown
            c.create_oval(x-2,y-s//2-1,x+2,y-s//2+3,fill="#ffee00",outline="")  # eye

        elif fid=="f_kraken":
            # Kraken rising from water
            c.create_rectangle(x-s,y+s//3,x+s,y+s,fill="#224488",outline="#112244",width=k)  # water
            # Wave detail
            for wx_ in range(x-s,x+s,8):
                c.create_arc(wx_,y+s//3-3,wx_+8,y+s//3+3,start=0,extent=180,outline="#3366aa",style="arc",width=1)
            c.create_oval(x-s//2,y-s//2,x+s//2,y+s//3+2,fill=col,outline="#112244",width=k)  # body
            # Tentacles
            for ti_ in range(6):
                ta_=math.radians(ti_*60); tx_=x+int(s*math.cos(ta_)); ty_=y+int(s*math.sin(ta_))
                c.create_line(x,y,tx_,ty_,fill=col,width=max(3,k+1))
                c.create_oval(tx_-4,ty_-4,tx_+4,ty_+4,fill="#334466",outline=col,width=1)
                # Suckers
                c.create_oval(int((x+tx_)//2)-2,int((y+ty_)//2)-2,int((x+tx_)//2)+2,int((y+ty_)//2)+2,fill="#aaccdd",outline="")
            # Eyes
            c.create_oval(x-s//4-4,y-s//4,x-s//4+4,y-s//4+8,fill="#ffee00",outline="#111",width=k)
            c.create_oval(x+s//4-4,y-s//4,x+s//4+4,y-s//4+8,fill="#ffee00",outline="#111",width=k)
            c.create_oval(x-s//4-1,y-s//4+2,x-s//4+1,y-s//4+5,fill="#111",outline="")
            c.create_oval(x+s//4-1,y-s//4+2,x+s//4+1,y-s//4+5,fill="#111",outline="")

        elif fid=="f_golem":
            # Ancient stone golem with moss and runes
            c.create_oval(x-s//2,y-s,x+s//2,y-s//3,fill=col,outline="#554433",width=k+1)  # head
            # Face cracks
            c.create_line(x-s//4,y-s+4,x-s//4+2,y-s//2,fill="#443322",width=k)
            c.create_line(x+s//4,y-s+6,x+s//4-1,y-s//2+2,fill="#443322",width=k)
            c.create_rectangle(x-s+2,y-s//3,x+s-2,y+s//4,fill=col,outline="#554433",width=k+1)  # torso
            # Rune on chest
            c.create_text(x,y-s//8,text="᛭",fill="#66aa44",font=tkfont.Font(size=max(5,s//2)))
            # Mossy patches
            for mx_,my_ in [(-s+4,y-s//4),(s-8,y),(0,y+s//8)]:
                c.create_oval(x+mx_-3,my_-2,x+mx_+5,my_+4,fill="#3a7a20",outline="")
            c.create_rectangle(x-s//4,y+s//4,x+s//4,y+s,fill=col,outline="#554433",width=k)  # legs
            # Arms — massive slabs
            c.create_rectangle(x-s-6,y-s//4,x-s+2,y+s//4,fill="#887766",outline="#554433",width=k)
            c.create_rectangle(x+s-2,y-s//4,x+s+6,y+s//4,fill="#887766",outline="#554433",width=k)
            # Stone texture
            for tx_,ty_ in [(-s//2,y-s//5),(s//4,y-s//3),(0,y+s//8)]:
                c.create_oval(x+tx_-2,ty_-1,x+tx_+2,ty_+2,fill="#998877",outline="")

        # ── LEGENDARY ──────────────────────────────────
        elif fid=="f_horse_gold":
            col="#f5c518"
            mane_col="#fffaaa"
            c.create_oval(x-s+4,y-s+10,x+s-2,y+8,fill=col,outline="#c8a000",width=k+1)
            c.create_polygon(x-s+6,y-s+8,x-s+4,y-s-2,x-s+14,y-s,x-s+12,y-s+10,fill=col,outline="#c8a000",width=1)
            c.create_oval(x-s+2,y-s-4,x-s+16,y-s+8,fill=col,outline="#c8a000",width=k)
            c.create_oval(x-s+2,y-s+2,x-s+10,y-s+10,fill="#e8b818",outline="#a08000",width=1)
            c.create_oval(x-s+3,y-s+5,x-s+6,y-s+8,fill="#7a5a00",outline="")
            c.create_oval(x-s+9,y-s-1,x-s+14,y-s+4,fill="#ffee00",outline="#c8a000",width=1)
            c.create_oval(x-s+10,y-s,x-s+13,y-s+3,fill="#111",outline="")
            c.create_polygon(x-s+10,y-s-4,x-s+7,y-s-10,x-s+14,y-s-4,fill=col,outline="#c8a000")
            for mi_ in range(5):
                c.create_line(x-s+12-mi_*2,y-s-2,x-s+8-mi_*2,y-s+8,fill=mane_col,width=max(2,k+1))
            for li_,lx_ in enumerate([x-s+6,x-s+10,x+s-12,x+s-8]):
                c.create_line(lx_,y+6,lx_+(1 if li_%2 else -1)*2,y+s,fill=col,width=max(3,k+1))
                c.create_oval(lx_-2,y+s-2,lx_+4,y+s+4,fill="#c8a000",outline="")
            c.create_line(x+s-4,y-s+12,x+s+4,y-s,fill=mane_col,width=max(3,k+1))
            # Trophy star + sparkles
            for sp_ in range(6):
                sa_=math.radians(sp_*60)
                c.create_line(x,y-s//2,x+int(8*math.cos(sa_)),y-s//2+int(8*math.sin(sa_)),fill="#ffee44",width=1)
            c.create_oval(x-3,y-s//2-3,x+3,y-s//2+3,fill=GOLD,outline="#c8a000",width=1)

        elif fid=="f_ff_legend":
            # Legendary fighter engulfed in fire AND ice
            skin="#f8d0a0"
            c.create_oval(x-s//2,y-s,x+s//2,y-s//3,fill=skin,outline="#333",width=k)
            # Split hair — left fire, right ice
            for hi_ in range(x-s//3,x,4):
                c.create_line(hi_,y-s,hi_,y-s-6,fill="#ff4400",width=2)
            for hi_ in range(x,x+s//3+1,4):
                c.create_line(hi_,y-s,hi_,y-s-6,fill="#88aaff",width=2)
            c.create_rectangle(x-s//2+2,y-s//3,x+s//2-2,y+s//4,fill="#444",outline="#333",width=k)
            c.create_rectangle(x-s//2+2,y+s//4,x-2,y+s,fill="#444",outline="#333",width=k)
            c.create_rectangle(x+2,y+s//4,x+s//2-2,y+s,fill="#444",outline="#333",width=k)
            c.create_rectangle(x-s//2+1,y+s-3,x-1,y+s+3,fill="#111",outline="")
            c.create_rectangle(x+1,y+s-3,x+s//2-1,y+s+3,fill="#111",outline="")
            # Fire arm (right)
            c.create_line(x+s//2-2,y-s//4,x+s+4,y-s//2,fill=skin,width=max(3,k+1))
            for fi_ in range(4):
                c.create_oval(x+s+fi_*3,y-s//2-fi_*2,x+s+fi_*3+6,y-s//2+4-fi_*2,fill=["#ff2200","#ff6600","#ff8800","#ffcc00"][fi_],outline="")
            # Ice arm (left)
            c.create_line(x-s//2+2,y-s//4,x-s-4,y-s//2,fill=skin,width=max(3,k+1))
            c.create_polygon(x-s-6,y-s//2-4,x-s-10,y-s//2-8,x-s,y-s//2+4,fill="#88ddff",outline="#44aacc")
            # Dual aura rings
            c.create_oval(x-s-4,y-s-4,x+s+4,y+s+4,fill="",outline="#ff4400",width=1)
            c.create_oval(x-s-2,y-s-2,x+s+2,y+s+2,fill="",outline="#88aaff",width=1)

        elif fid=="f_serpent":
            # Giant sea serpent with multiple coils
            for ri_ in range(5):
                r_=s-ri_*(s//5)
                c.create_oval(x-r_,y-r_//2,x+r_,y+r_//2,fill="",outline=col,width=max(3,5-ri_))
            # Scale texture
            for ssx_ in range(-s+4,s-3,6):
                c.create_oval(x+ssx_-2,y-3,x+ssx_+2,y+1,fill="#224422",outline="")
            # Massive head
            c.create_oval(x+s-8,y-s//2-6,x+s+6,y-s//2+6,fill=col,outline="#006644",width=k+1)
            c.create_oval(x+s-4,y-s//2-3,x+s+1,y-s//2+2,fill="#111",outline="")
            c.create_oval(x+s-3,y-s//2-2,x+s-1,y-s//2,fill="#ffcc00",outline="")
            c.create_line(x+s+6,y-s//2,x+s+10,y-s//2-3,fill="#cc0000",width=1)
            c.create_line(x+s+6,y-s//2,x+s+10,y-s//2+3,fill="#cc0000",width=1)

        elif fid=="f_leviathan":
            # Colossal ocean beast
            for ri_ in range(6):
                r_=s-ri_*(s//6)
                c.create_oval(x-r_,y-r_//2,x+r_,y+r_//2,fill="",outline=col,width=max(2,6-ri_))
            c.create_oval(x+s-8,y-s//3-5,x+s+5,y-s//3+5,fill=col,outline="#001a22",width=k+1)
            for bi_,bx_ in enumerate([x+s-6,x+s-3,x+s+1]):
                c.create_oval(bx_,y-s//3-2,bx_+3,y-s//3+2,fill=["#ffee00","#ff6600","#ff2200"][bi_],outline="")
            # Spine fins
            for fi_,(fx_,fy_) in enumerate([(x-s//2,y-s//2),(x,y-s//2),(x+s//2,y-s//2)]):
                c.create_polygon(fx_,fy_,fx_-4,fy_-8+fi_*3,fx_+4,fy_,fill=col,outline="#001a22")

        elif fid=="f_colossus":
            # Giant iron warrior
            c.create_oval(x-s//2,y-s,x+s//2,y-s//3,fill=col,outline="#665544",width=k+1)  # head
            # Helmet visor
            c.create_rectangle(x-s//3,y-s//2,x+s//3,y-s//3+2,fill="#555",outline="#443322",width=1)
            c.create_rectangle(x-s+2,y-s//3,x+s-2,y+s//4,fill=col,outline="#665544",width=k+1)  # torso
            # Breastplate detail
            c.create_polygon(x-s//2,y-s//3+2,x,y-s//6,x+s//2,y-s//3+2,fill="#887766",outline="")
            c.create_oval(x-4,y-s//6-2,x+4,y-s//6+6,fill="#cc8800",outline="#886600",width=1)
            c.create_rectangle(x-s//2,y+s//4,x-s//4,y+s,fill=col,outline="#665544",width=k)  # leg L
            c.create_rectangle(x+s//4,y+s//4,x+s//2,y+s,fill=col,outline="#665544",width=k)  # leg R
            # Boots
            c.create_rectangle(x-s//2,y+s-3,x-s//4,y+s+4,fill="#333",outline="#222",width=1)
            c.create_rectangle(x+s//4,y+s-3,x+s//2,y+s+4,fill="#333",outline="#222",width=1)
            # Arms
            c.create_rectangle(x-s-6,y-s//3,x-s+2,y+s//4,fill=col,outline="#665544",width=k)
            c.create_rectangle(x+s-2,y-s//3,x+s+6,y+s//4,fill=col,outline="#665544",width=k)
            # Gauntlets
            c.create_rectangle(x-s-7,y+s//8,x-s+3,y+s//4+2,fill="#556655",outline="#334433",width=1)
            c.create_rectangle(x+s-3,y+s//8,x+s+7,y+s//4+2,fill="#556655",outline="#334433",width=1)

        elif fid=="f_unicorn":
            # Unicorn with detailed mane and rainbow horn
            c.create_oval(x-s+4,y-s+10,x+s-2,y+8,fill=col,outline="#cc88ff",width=k)
            c.create_polygon(x-s+6,y-s+8,x-s+4,y-s-2,x-s+14,y-s,x-s+12,y-s+10,fill=col,outline="#cc88ff",width=1)
            c.create_oval(x-s+2,y-s-4,x-s+16,y-s+8,fill=col,outline="#cc88ff",width=k)
            c.create_oval(x-s+9,y-s-1,x-s+14,y-s+4,fill="#ffbbff",outline="#cc88ff",width=1)
            c.create_oval(x-s+10,y-s,x-s+13,y-s+3,fill="#111",outline="")
            # Ear
            c.create_polygon(x-s+10,y-s-4,x-s+7,y-s-10,x-s+14,y-s-4,fill=col,outline="#cc88ff")
            # Rainbow mane
            mane_cols=["#ff4444","#ffaa00","#ffff00","#44ff44","#44aaff","#cc44ff"]
            for mi_,mc_ in enumerate(mane_cols):
                c.create_line(x-s+12-mi_*2,y-s-2,x-s+8-mi_*2,y-s+8,fill=mc_,width=2)
            for li_,lx_ in enumerate([x-s+6,x-s+10,x+s-12,x+s-8]):
                c.create_line(lx_,y+6,lx_+(1 if li_%2 else -1)*2,y+s,fill=col,width=max(3,k+1))
                c.create_oval(lx_-2,y+s-2,lx_+4,y+s+4,fill="#cc88ff",outline="")
            c.create_line(x+s-4,y-s+12,x+s+4,y-s,fill="#ff88ff",width=max(3,k+1))
            # Spiral rainbow horn
            for hi_ in range(5):
                c.create_line(x-s+6-hi_,y-s+2-hi_*2,x-s+4-hi_,y-s-4-hi_*2,fill=mane_cols[hi_],width=max(2,k))
            c.create_polygon(x-s+6,y-s+2,x-s+4,y-s+2,x-s,y-s-10,fill=GOLD,outline="")
            # Star sparkles around
            for sp_,sc_ in enumerate(["#ff44ff","#ffff44","#44ffff"]):
                sa_=math.radians(sp_*120+30)
                c.create_text(x+int((s+4)*math.cos(sa_)),y+int((s+4)*math.sin(sa_))-s//3,text="★",fill=sc_,font=tkfont.Font(size=max(4,s//3)))

        elif fid=="f_sphinx":
            # Desert sphinx with headdress
            c.create_oval(x-s+2,y-s//4,x+s-2,y+s,fill=col,outline="#8a6820",width=k)  # body/paws
            c.create_oval(x-s//2,y-s,x+s//2,y-s//4,fill=col,outline="#8a6820",width=k)  # head
            # Nemes headdress
            c.create_rectangle(x-s//2,y-s,x+s//2,y-s//2,fill="#ddaa00",outline="#aa7800",width=k)
            c.create_polygon(x-s//2,y-s+2,x-s//2-4,y-s//4,x-s//2+4,y-s//4,fill="#ddaa00",outline="#aa7800")
            c.create_polygon(x+s//2,y-s+2,x+s//2+4,y-s//4,x+s//2-4,y-s//4,fill="#ddaa00",outline="#aa7800")
            # Uraeus (cobra on forehead)
            c.create_oval(x-4,y-s+2,x+4,y-s+10,fill="#cc0000",outline="#880000",width=1)
            c.create_polygon(x-3,y-s+8,x-s//8,y-s+4,x+s//8,y-s+4,x+3,y-s+8,fill="#cc0000",outline="")
            # Headdress stripes
            for hi_,hc_ in enumerate(["#cc9900","#ddbb22","#cc9900"]):
                c.create_line(x-s//2+2,y-s+6+hi_*8,x+s//2-2,y-s+6+hi_*8,fill=hc_,width=1)
            # Eyes (kohl-lined)
            c.create_oval(x-s//4-3,y-s//2-1,x-s//4+3,y-s//2+5,fill="#111",outline="#444",width=k)
            c.create_oval(x+s//4-3,y-s//2-1,x+s//4+3,y-s//2+5,fill="#111",outline="#444",width=k)
            c.create_line(x-s//4-5,y-s//2+2,x-s//4+5,y-s//2+2,fill="#222",width=1)
            c.create_line(x+s//4-5,y-s//2+2,x+s//4+5,y-s//2+2,fill="#222",width=1)
            # Paws
            for px_ in [-s+4,s-8]:
                c.create_oval(x+px_,y+s//2,x+px_+8,y+s,fill=col,outline="#8a6820",width=1)

        elif fid=="f_valkyrie":
            # Norse valkyrie descending
            skin="#f0d0a0"
            c.create_oval(x-s//2,y-s,x+s//2,y-s//3,fill=skin,outline="#aaa",width=k)  # head
            # Winged helmet
            c.create_rectangle(x-s//2,y-s,x+s//2,y-s+s//4,fill="#8888aa",outline="#555588",width=k)
            c.create_polygon(x-s//2,y-s,x-s-4,y-s-6,x-s//4,y-s,fill="#aaaacc",outline="#666688")  # wing L
            c.create_polygon(x+s//2,y-s,x+s+4,y-s-6,x+s//4,y-s,fill="#aaaacc",outline="#666688")  # wing R
            # Armour torso
            c.create_rectangle(x-s//2+2,y-s//3,x+s//2-2,y+s//4,fill=col,outline=rc,width=k)
            # Chainmail texture
            for cy_ in range(y-s//3+2,y+s//4,4):
                for cx_ in range(x-s//2+3,x+s//2-2,6):
                    c.create_oval(cx_,cy_,cx_+4,cy_+3,fill="",outline="#888",width=1)
            c.create_rectangle(x-s//2,y+s//4,x-s//4,y+s,fill=col,outline=rc,width=k)
            c.create_rectangle(x+s//4,y+s//4,x+s//2,y+s,fill=col,outline=rc,width=k)
            # Greaves
            c.create_rectangle(x-s//2,y+s-4,x-s//4,y+s+4,fill="#8888aa",outline="#555588",width=1)
            c.create_rectangle(x+s//4,y+s-4,x+s//2,y+s+4,fill="#8888aa",outline="#555588",width=1)
            # Shield (left arm)
            c.create_oval(x-s-4,y-s//4,x-s+8,y+s//4+4,fill="#8888aa",outline="#555566",width=k)
            c.create_polygon(x-s+2,y-s//4+4,x-s-2,y,x-s+2,y+s//4,fill="#6666aa",outline="")
            # Spear (right arm)
            c.create_line(x+s//2-2,y-s//4,x+s+8,y-s-s,fill=GOLD,width=k+1)
            c.create_polygon(x+s+6,y-s-s,x+s+2,y-s-s+6,x+s+10,y-s-s+4,fill=GOLD,outline="#c8a000")

        elif fid=="f_titan":
            # Stone titan crouching — massive
            c.create_oval(x-s//2,y-s,x+s//2,y-s//3,fill=col,outline="#444433",width=k+1)  # head
            # Craggy face
            c.create_line(x-s//3,y-s+4,x-s//4,y-s//2,fill="#333322",width=k)
            c.create_line(x+s//4,y-s+6,x+s//3,y-s//2,fill="#333322",width=k)
            c.create_line(x-s//4,y-s//2+2,x+s//4,y-s//2+2,fill="#333322",width=k)  # grim mouth
            # Massive torso
            c.create_rectangle(x-s,y-s//3,x+s,y+s//3,fill=col,outline="#444433",width=k+1)
            # Pectoral carvings
            c.create_polygon(x-s+4,y-s//3+4,x-4,y-s//6,x-s+4,y+s//6,fill="#665544",outline="")
            c.create_polygon(x+4,y-s//3+4,x+s-4,y-s//6,x+4,y+s//6,fill="#665544",outline="")
            # Crouching legs — wide
            c.create_rectangle(x-s,y+s//3,x-s//3,y+s,fill=col,outline="#444433",width=k)
            c.create_rectangle(x+s//3,y+s//3,x+s,y+s,fill=col,outline="#444433",width=k)
            # Huge fists on ground
            c.create_oval(x-s-6,y+s//4,x-s+6,y+s//4+12,fill="#887766",outline="#554433",width=k)
            c.create_oval(x+s-6,y+s//4,x+s+6,y+s//4+12,fill="#887766",outline="#554433",width=k)
            # Rock chunks around base
            for ri_,(rx_,ry_) in enumerate([(-s-4,y+s//2),(s+4,y+s//2),(-s//2,y+s+4),(s//2,y+s+4)]):
                c.create_polygon(x+rx_,ry_,x+rx_+4,ry_-4,x+rx_+8,ry_,x+rx_+4,ry_+4,fill="#666655",outline="#444433")
            # Mossy bits
            for mx_,my_ in [(-s,y-s//6),(s-4,y),(0,y+s//3)]:
                c.create_oval(x+mx_,my_,x+mx_+6,my_+4,fill="#3a7a20",outline="")

        # ── MYTHIC ───────────────────────────────────────
        elif fid=="f_myth_rg":
            # RG Deity — glowing all-seeing radiant entity
            # Outer radiance
            for ri_ in range(12):
                ra_=math.radians(ri_*30)
                c.create_line(x+int((s//2)*math.cos(ra_)),y+int((s//2)*math.sin(ra_)),
                              x+int((s+4)*math.cos(ra_)),y+int((s+4)*math.sin(ra_)),fill=col,width=k+1)
                c.create_oval(x+int((s+2)*math.cos(ra_))-2,y+int((s+2)*math.sin(ra_))-2,
                              x+int((s+2)*math.cos(ra_))+2,y+int((s+2)*math.sin(ra_))+2,fill=rc,outline="")
            # Body — ethereal pillar
            c.create_oval(x-s//3,y-s,x+s//3,y+s,fill=col,outline=rc,width=k+1)
            # Inner glow layers
            for li_,lc_ in enumerate([col,"#ffffff","#ffeecc"]):
                r_=s//3-li_*4
                if r_>0: c.create_oval(x-r_,y-r_,x+r_,y+r_,fill=lc_,outline="")
            # All-seeing eye
            c.create_oval(x-s//4,y-s//4,x+s//4,y+s//4,fill="#ffffff",outline=rc,width=k)
            c.create_oval(x-s//6,y-s//6,x+s//6,y+s//6,fill="#0000aa",outline="")
            c.create_oval(x-s//10,y-s//10,x+s//10,y+s//10,fill="#111",outline="")
            c.create_text(x,y-s//2,text="R",fill=rc,font=tkfont.Font(family="Georgia",size=max(5,s//2),weight="bold"))
            c.create_text(x,y+s//2,text="G",fill=rc,font=tkfont.Font(family="Georgia",size=max(5,s//2),weight="bold"))

        elif fid=="f_myth_time":
            # Timekeeper — ancient clockwork
            c.create_oval(x-s,y-s,x+s,y+s,fill=col,outline=rc,width=k+1)  # face
            c.create_oval(x-s+3,y-s+3,x+s-3,y+s-3,fill="#111122",outline="#665544",width=k)
            # Hour markers
            for hi_ in range(12):
                ha_=math.radians(hi_*30-90)
                inner=s-7; outer=s-4
                c.create_line(x+int(inner*math.cos(ha_)),y+int(inner*math.sin(ha_)),
                              x+int(outer*math.cos(ha_)),y+int(outer*math.sin(ha_)),fill=rc,width=k)
            # Hands
            c.create_line(x,y,x,y-s+8,fill=col,width=k+2)   # hour
            c.create_line(x,y,x+s-6,y+4,fill=rc,width=k)    # minute
            c.create_oval(x-3,y-3,x+3,y+3,fill=GOLD,outline="#c8a000",width=1)
            # Roman numerals at 12,3,6,9
            for pos_,num_ in [((0,-s+10),"XII"),((s-10,4),"III"),((0,s-6),"VI"),((-s+6,4),"IX")]:
                c.create_text(x+pos_[0],y+pos_[1],text=num_,fill=rc,font=tkfont.Font(size=max(4,s//4)))
            # Outer gears
            for gi_,ga_ in enumerate([0,90,180,270]):
                gx2_=x+int(s*math.cos(math.radians(ga_))); gy2_=y+int(s*math.sin(math.radians(ga_)))
                c.create_oval(gx2_-6,gy2_-6,gx2_+6,gy2_+6,fill="#2a2a2a",outline="#888",width=1)
                for tt_ in range(8):
                    ta_=math.radians(tt_*45+gi_*15)
                    c.create_line(gx2_+int(5*math.cos(ta_)),gy2_+int(5*math.sin(ta_)),
                                  gx2_+int(7*math.cos(ta_)),gy2_+int(7*math.sin(ta_)),fill="#aaa",width=1)

        elif fid=="f_myth_void":
            # Void Serpent — ouroboros
            thick=max(4,s//3)
            c.create_oval(x-s+2,y-s+2,x+s-2,y+s-2,fill="",outline=col,width=thick)
            # Scale pattern on ring
            for si_ in range(0,360,20):
                sa_=math.radians(si_); mid_r=s-thick//2
                c.create_oval(x+int(mid_r*math.cos(sa_))-2,y+int(mid_r*math.sin(sa_))-2,
                              x+int(mid_r*math.cos(sa_))+2,y+int(mid_r*math.sin(sa_))+2,fill="#330033",outline="")
            # Head eating tail at right
            c.create_oval(x+s-8,y-5,x+s+4,y+5,fill=col,outline=rc,width=k)
            c.create_oval(x+s-4,y-3,x+s,y,fill="#cc0000",outline="")
            c.create_oval(x+s-3,y,x+s-1,y+3,fill="#cc0000",outline="")
            # Void eyes
            c.create_oval(x+s-6,y-3,x+s-3,y,fill="#ffffff",outline="")
            c.create_oval(x+s-5,y-2,x+s-4,y-1,fill="#111",outline="")
            # Inner void star
            for vi_ in range(6):
                va_=math.radians(vi_*60)
                c.create_line(x,y,x+int((s-thick-2)*math.cos(va_)),y+int((s-thick-2)*math.sin(va_)),fill="#440044",width=k)
            c.create_oval(x-s//4,y-s//4,x+s//4,y+s//4,fill="#110011",outline="#660066",width=k)

        elif fid=="f_myth_sun":
            # Solar Dragon — dragon made of sunlight
            # Sunbeam rays
            for ri_ in range(16):
                ra_=math.radians(ri_*22.5)
                c.create_line(x+int((s//2)*math.cos(ra_)),y+int((s//2)*math.sin(ra_)),
                              x+int((s+5)*math.cos(ra_)),y+int((s+5)*math.sin(ra_)),
                              fill=["#ffee00","#ff8800"][ri_%2],width=k)
            c.create_oval(x-s//2,y-s//2,x+s//2,y+s//2,fill=col,outline="#ff8800",width=k+1)
            # Dragon silhouette in sun
            c.create_polygon(x,y-s//2,x-s//3,y+s//4,x,y,x+s//3,y+s//4,fill="#cc8800",outline="#886600",width=k)
            c.create_line(x,y-s//2,x-s//2-2,y-s//4,fill="#cc8800",width=k+1)
            c.create_line(x,y-s//2,x+s//2+2,y-s//4,fill="#cc8800",width=k+1)
            c.create_oval(x-s//5,y-s//2,x+s//5,y-s//5,fill=col,outline="#cc8800",width=k)
            # Solar corona
            c.create_oval(x-s,y-s,x+s,y+s,fill="",outline="#ffee00",width=k)
            c.create_oval(x-s-3,y-s-3,x+s+3,y+s+3,fill="",outline="#ff8800",width=1)

        elif fid=="f_myth_chs":
            # Chaos God — all rarities swirling
            # Rarity colour arcs
            rarity_list=list(self.RARITY_COLS.items())
            for ci_,(rar_,rcol_) in enumerate(rarity_list):
                arc_s=s-ci_*2 if s-ci_*2>4 else 4
                c.create_arc(x-arc_s,y-arc_s,x+arc_s,y+arc_s,
                             start=ci_*60,extent=55,outline=rcol_,style="arc",width=max(2,k+1))
            # Chaotic body — distorted humanoid
            c.create_oval(x-s//2,y-s,x+s//2,y-s//3,fill="#220022",outline=rc,width=k)
            c.create_rectangle(x-s//2+2,y-s//3,x+s//2-2,y+s//4,fill="#1a001a",outline=rc,width=k)
            # Extra chaotic arms
            for ai_,ac_ in enumerate(["#ffaa00","#4488ff","#ff4444","#44ff88"]):
                aa_=math.radians(ai_*90+45)
                c.create_line(x,y-s//8,x+int((s+4)*math.cos(aa_)),y-s//8+int((s+4)*math.sin(aa_)),fill=ac_,width=k+1)
                c.create_oval(x+int((s+2)*math.cos(aa_))-3,y-s//8+int((s+2)*math.sin(aa_))-3,
                              x+int((s+2)*math.cos(aa_))+3,y-s//8+int((s+2)*math.sin(aa_))+3,fill=ac_,outline="")
            # Eyes — 3 of them
            c.create_oval(x-s//4-3,y-s//2-1,x-s//4+3,y-s//2+5,fill="#ff0000",outline="#111",width=1)
            c.create_oval(x+s//4-3,y-s//2-1,x+s//4+3,y-s//2+5,fill="#00ff44",outline="#111",width=1)
            c.create_oval(x-2,y-s+3,x+2,y-s+9,fill="#0044ff",outline="#111",width=1)
            # Central vortex
            c.create_oval(x-s//4,y-s//4,x+s//4,y+s//4,fill="",outline="#ff00ff",width=k)
            c.create_oval(x-s//6,y-s//6,x+s//6,y+s//6,fill="",outline="#ffff00",width=k)
            c.create_oval(x-s//10,y-s//10,x+s//10,y+s//10,fill="#ff00ff",outline="")

        else:
            # Fallback
            c.create_oval(x-s//2,y-s,x+s//2,y-s//3,fill=col,outline="#333",width=k)
            c.create_rectangle(x-s//2+2,y-s//3,x+s//2-2,y+s//3,fill=col,outline="#333",width=k)
            c.create_line(x-s//2+2,y-s//4,x-s,y+s//6,fill=col,width=k+1)
            c.create_line(x+s//2-2,y-s//4,x+s,y+s//6,fill=col,width=k+1)
            c.create_line(x-4,y+s//3,x-6,y+s,fill=col,width=k+1)
            c.create_line(x+4,y+s//3,x+6,y+s,fill=col,width=k+1)

        # Rarity dot at bottom
        c.create_oval(x-4,y+s,x+4,y+s+8,fill=rc,outline="")

        # ── COMMON ────────────────────────────────────────
        if fid=="f_coin":
            c.create_oval(x-s,y-s,x+s,y+s,fill=col,outline="#b8a000",width=2)
            c.create_oval(x-s+3,y-s+3,x+s-3,y+s-3,fill="#e8c000",outline="")
            c.create_text(x,y,text="$",fill="#7a6000",font=tkfont.Font(size=max(6,s-2),weight="bold"))
        elif fid=="f_dice":
            c.create_rectangle(x-s,y-s,x+s,y+s,fill=col,outline="#999",width=2)
            for dx_,dy_ in [(-s//2,-s//2),(0,-s//2),(s//2,-s//2),(-s//2,0),(s//2,0),(0,s//2)]:
                c.create_oval(x+dx_-2,y+dy_-2,x+dx_+2,y+dy_+2,fill="#222",outline="")
        elif fid=="f_card":
            c.create_rectangle(x-s+2,y-s,x+s-2,y+s,fill=col,outline="#555",width=1)
            c.create_text(x,y-s//2,text="A",fill=rc,font=tkfont.Font(size=max(6,s),weight="bold"))
            c.create_text(x,y+s//4,text="♠",fill=rc,font=tkfont.Font(size=max(5,s-2)))
        elif fid=="f_horseshoe":
            c.create_arc(x-s,y-s//2,x+s,y+s+s//2,start=0,extent=180,outline=col,style="arc",width=max(3,s//3))
            c.create_line(x-s,y+s//2,x-s,y+s,fill=col,width=max(3,s//3))
            c.create_line(x+s,y+s//2,x+s,y+s,fill=col,width=max(3,s//3))
        elif fid=="f_mug":
            c.create_rectangle(x-s+2,y-s//2,x+s-2,y+s,fill=col,outline="#888",width=2)
            c.create_arc(x+s-4,y-s//4,x+s+s//2,y+s//2,start=270,extent=180,outline="#888",style="arc",width=2)
            c.create_rectangle(x-s+4,y-s//2,x+s-4,y-s//2+4,fill="#e8e0d0",outline="")
        elif fid=="f_cactus":
            c.create_rectangle(x-3,y-s,x+3,y+s,fill=col,outline="#1a5a10",width=1)
            c.create_rectangle(x-s+2,y-s//4,x-3,y,fill=col,outline="#1a5a10",width=1)
            c.create_rectangle(x+3,y-s//2,x+s-2,y+s//4,fill=col,outline="#1a5a10",width=1)
            c.create_oval(x-5,y-s-2,x+5,y-s+6,fill="#ffaaaa",outline="")
        elif fid=="f_cat":
            c.create_oval(x-s+2,y,x+s-2,y+s,fill=col,outline="#555",width=1)  # body
            c.create_oval(x-s//2,y-s,x+s//2,y+2,fill=col,outline="#555",width=1)  # head
            c.create_polygon(x-s//2,y-s+2,x-s//2-4,y-s-6,x-s//2+6,y-s+2,fill=col,outline="")  # ear L
            c.create_polygon(x+s//2,y-s+2,x+s//2+4,y-s-6,x+s//2-6,y-s+2,fill=col,outline="")  # ear R
            c.create_line(x+s-2,y+s//2,x+s+s//2,y+s//4,fill=col,width=2)  # tail
        elif fid=="f_parrot":
            c.create_oval(x-s//2,y-s,x+s//2,y,fill=col,outline="#1a6a10",width=1)
            c.create_oval(x-s+2,y-s//4,x+s-2,y+s,fill="#cc6600",outline="#885500",width=1)
            c.create_polygon(x-4,y-s//4,x-s,y+s//3,x+4,y-s//4,fill="#ffcc00",outline="")
            c.create_oval(x-3,y-s//2,x+3,y-s//2+6,fill="#111",outline="")
        elif fid=="f_boot":
            c.create_rectangle(x-s+2,y-s//2,x+s//4,y+s,fill=col,outline="#3a2008",width=2)
            c.create_rectangle(x-s+2,y+s//2,x+s,y+s,fill=col,outline="#3a2008",width=2)
            c.create_oval(x-s,y-s,x-s+8,y-s+10,fill=col,outline="#3a2008",width=1)
        elif fid=="f_star":
            pts=[]
            for si2 in range(5):
                a_out=math.radians(si2*72-90); a_in=math.radians(si2*72+36-90)
                pts+=[x+int(s*math.cos(a_out)),y+int(s*math.sin(a_out)),
                      x+int(s//2*math.cos(a_in)),y+int(s//2*math.sin(a_in))]
            c.create_polygon(pts,fill=col,outline="#aa8800",width=1)
        elif fid=="f_mushroom":
            c.create_arc(x-s,y-s,x+s,y+4,start=0,extent=180,fill=col,outline="#881100",width=2)
            c.create_rectangle(x-s//2,y+2,x+s//2,y+s,fill="#e8e0d0",outline="#aaa",width=1)
            for mx_ in [-s//2+2,0,s//2-2]:
                c.create_oval(x+mx_-4,y-s//2-2,x+mx_+4,y-s//2+6,fill="#ffffff",outline="")
        elif fid=="f_snake_sm":
            for ri_ in range(3):
                r_=s-ri_*4
                c.create_oval(x-r_,y-r_//2,x+r_,y+r_//2,fill="",outline=col,width=max(2,3-ri_))
            c.create_oval(x+s-6,y-s//2-4,x+s+2,y-s//2+4,fill=col,outline="#1a4a10",width=1)
            c.create_oval(x+s-4,y-s//2-1,x+s-1,y-s//2+2,fill="#cc0000",outline="")
        elif fid=="f_frog":
            c.create_oval(x-s+2,y-s//3,x+s-2,y+s-2,fill=col,outline="#1a6a10",width=1)
            c.create_oval(x-s+2,y-s,x-s+10,y-s+8,fill=col,outline="#1a6a10",width=1)
            c.create_oval(x+s-10,y-s,x+s-2,y-s+8,fill=col,outline="#1a6a10",width=1)
            c.create_oval(x-4,y-s//2+2,x,y-s//2+6,fill="#cc0000",outline="")
            c.create_oval(x+1,y-s//2+2,x+5,y-s//2+6,fill="#cc0000",outline="")
        elif fid=="f_candle":
            c.create_rectangle(x-s//3,y,x+s//3,y+s,fill=col,outline="#888",width=1)
            for dxx in range(-s//3+1,s//3,3):
                c.create_rectangle(x+dxx,y,x+dxx+2,y+s,fill="#e8d890",outline="")
            c.create_polygon(x,y-s,x-4,y,x+4,y,fill="#ffcc44",outline="")
            c.create_oval(x-2,y-4,x+2,y+1,fill="#ff8800",outline="")
        elif fid=="f_anchor":
            c.create_oval(x-s//3,y-s,x+s//3,y-s//3,fill="",outline=col,width=max(2,s//5))
            c.create_line(x,y-s//3,x,y+s-4,fill=col,width=max(2,s//5))
            c.create_line(x-s+2,y,x+s-2,y,fill=col,width=max(2,s//5))
            c.create_arc(x-s+2,y+s//2-4,x,y+s+4,start=180,extent=90,outline=col,style="arc",width=max(2,s//5))
            c.create_arc(x,y+s//2-4,x+s-2,y+s+4,start=270,extent=90,outline=col,style="arc",width=max(2,s//5))
        elif fid=="f_lantern":
            c.create_oval(x-s+2,y-s//2,x+s-2,y+s-2,fill=col,outline="#882200",width=2)
            c.create_rectangle(x-3,y-s,x+3,y-s//2,fill="#888",outline="")
            for fx_ in [x-s+4,x-s//2,x,x+s//2-2,x+s-6]:
                c.create_line(fx_,y-s//2,fx_,y+s-4,fill="#cc4400",width=1)
            c.create_oval(x-s//2,y-s//4,x+s//2,y+s//2,fill="#ff8800",outline="")
        elif fid=="f_clover":
            for ca_ in [0,90,180,270]:
                cx_=x+int(s//3*math.cos(math.radians(ca_))); cy_=y+int(s//3*math.sin(math.radians(ca_)))
                c.create_oval(cx_-s//2,cy_-s//2,cx_+s//2,cy_+s//2,fill=col,outline="#1a6a10",width=1)
            c.create_line(x,y,x,y+s,fill="#2a7a20",width=2)
        elif fid=="f_barrel":
            c.create_rectangle(x-s+2,y-s//2,x+s-2,y+s,fill=col,outline="#3a1800",width=2)
            for hy_ in [y-s//4,y+s//4]:
                c.create_line(x-s+2,hy_,x+s-2,hy_,fill="#5a2800",width=2)
            c.create_oval(x-s+2,y-s//2-2,x+s-2,y-s//2+6,fill="#8B3000",outline="#3a1800",width=1)
        elif fid=="f_spade":
            pts2=[x,y-s,x-s+2,y+2,x-s//3,y+2,x-s//3,y+s,x+s//3,y+s,x+s//3,y+2,x+s-2,y+2]
            c.create_polygon(pts2,fill=col,outline="#333",width=1)
        elif fid=="f_gem_sm":
            c.create_polygon(x,y-s,x-s+2,y-2,x-s//2,y+s-2,x+s//2,y+s-2,x+s-2,y-2,fill=col,outline="#3366aa",width=1)
            c.create_line(x,y-s,x-s+2,y-2,fill="#88aaff",width=1)
            c.create_line(x,y-s,x+s-2,y-2,fill="#88aaff",width=1)

        # ── UNCOMMON (horses + misc) ───────────────────────
        elif "horse" in fid:
            # Distinct horse per variant
            variant=fid.split("_")[-1]
            c.create_oval(x-s+4,y-s+8,x+s-2,y+6,fill=col,outline="#333",width=1)  # body
            c.create_oval(x-s+2,y-s-2,x-s+12,y-s+10,fill=col,outline="#333",width=1)  # head
            neck_col=col
            c.create_line(x-s+8,y-s+8,x-s+4,y-s+2,fill=neck_col,width=3)
            for li_,lx_ in enumerate([x-s+6,x-s+10,x+s-12,x+s-8]):
                c.create_line(lx_,y+6,lx_+(1 if li_%2 else -1)*2,y+s,fill=col,width=3)
            c.create_line(x+s-4,y-s+10,x+s+4,y-s,fill=col,width=3)  # tail
            # Mane colour by variant
            mane={"tb":"#ff8800","nm":"#8888ff","ls":"#ffffaa","im":"#888888","sb":"#dd88ff","gold":"#fff700"}.get(variant,rc)
            c.create_line(x-s+4,y-s+2,x-s+10,y-s+8,fill=mane,width=3)
            if variant=="gold":  # trophy sparkles
                c.create_text(x+s-4,y-s,text="★",fill=GOLD,font=tkfont.Font(size=max(4,s//2)))
        elif fid=="f_casino_sm":
            c.create_rectangle(x-s+2,y-2,x+s-2,y+s,fill=col,outline="#3d0a0a",width=1)
            c.create_polygon(x-s+2,y-2,x,y-s+2,x+s-2,y-2,fill="#cc7700",outline="#885500",width=1)
            c.create_rectangle(x-4,y+2,x+4,y+s-2,fill="#0a0028",outline="")
        elif fid=="f_stable_sm":
            c.create_rectangle(x-s+2,y,x+s-2,y+s,fill=col,outline="#3a1800",width=1)
            c.create_polygon(x-s+2,y,x,y-s+4,x+s-2,y,fill="#5a2800",outline="#3a1800",width=1)
            c.create_rectangle(x-4,y+s//3,x+4,y+s,fill="#2a1000",outline="")
            c.create_line(x,y+s//3,x,y+s,fill="#3a1800",width=1)
        elif fid=="f_bank_sm":
            c.create_rectangle(x-s+2,y-2,x+s-2,y+s,fill=col,outline="#1a2a4a",width=1)
            for bpx_ in [x-s+4,x+s-4]:
                c.create_rectangle(bpx_-2,y-2,bpx_+2,y+s,fill="#d0c8b0",outline="")
            c.create_polygon(x-s,y-2,x,y-s,x+s,y-2,fill="#2a3a6a",outline="#1a2a4a",width=1)
            c.create_oval(x-4,y+s//3,x+4,y+s//3+8,fill="#888",outline="#555",width=1)
        elif fid=="f_shop_sm":
            c.create_rectangle(x-s+2,y-2,x+s-2,y+s,fill=col,outline="#1a5a3a",width=1)
            c.create_rectangle(x-s+2,y-2,x+s-2,y+s//3,fill="#2a7a5a",outline="")
            c.create_rectangle(x-4,y+s//3,x+4,y+s,fill="#2a1800",outline="")
        elif fid in ("f_snake_co","f_snake_py"):
            # Cobra rearing vs python coiled
            if fid=="f_snake_co":
                c.create_line(x,y+s,x,y-s//2,fill=col,width=max(3,s//3))
                c.create_oval(x-s//2,y-s,x+s//2,y-s//3,fill=col,outline="#886600",width=1)  # hood
                c.create_oval(x-3,y-s+s//4,x+3,y-s+s//4+6,fill=col,outline="#333",width=1)  # head
                c.create_text(x,y-s+4,text="👁",font=tkfont.Font(size=max(4,s//3)))
            else:
                for ri_ in range(3):
                    r_=s-ri_*5
                    c.create_oval(x-r_,y-r_//2,x+r_,y+r_//2,fill="",outline=col,width=max(2,4-ri_))
                c.create_oval(x+s-6,y-4,x+s+2,y+4,fill=col,outline="#2a5a10",width=1)
        elif fid=="f_wolf":
            c.create_oval(x-s+2,y-s//3,x+s-2,y+s,fill=col,outline="#777",width=1)
            c.create_oval(x-s//2,y-s,x+s//2,y-s//3+4,fill=col,outline="#777",width=1)
            c.create_polygon(x-s//2+2,y-s+2,x-s//2-4,y-s-8,x-s//2+8,y-s+4,fill=col,outline="")
            c.create_polygon(x+s//2-2,y-s+2,x+s//2+4,y-s-8,x+s//2-8,y-s+4,fill=col,outline="")
            c.create_oval(x-4,y-s//3+2,x,y-s//3+7,fill="#cc2200",outline="")
            c.create_oval(x+1,y-s//3+2,x+5,y-s//3+7,fill="#cc2200",outline="")
        elif fid=="f_owl":
            c.create_oval(x-s+2,y-s//3,x+s-2,y+s,fill=col,outline="#6a5030",width=1)
            c.create_oval(x-s//2,y-s,x+s//2,y-s//4,fill=col,outline="#6a5030",width=1)
            c.create_oval(x-s//2+2,y-s+2,x-s//2+8,y-s+2+s//3,fill="#111",outline="")
            c.create_oval(x+s//2-8,y-s+2,x+s//2-2,y-s+2+s//3,fill="#111",outline="")
            c.create_polygon(x-3,y-s//2,x,y-s//2-6,x+3,y-s//2,fill="#cc8800",outline="")
        elif fid=="f_crystal":
            c.create_polygon(x,y-s,x-s+4,y,x-s//2,y+s-2,x+s//2,y+s-2,x+s-4,y,fill=col,outline="#6688aa",width=1)
            c.create_line(x,y-s,x-s+4,y,fill="#aaccff",width=1)
            c.create_line(x,y-s,x+s-4,y,fill="#aaccff",width=1)
            c.create_oval(x-s//3,y-s//3,x+s//3,y+s//3,fill="#aaccff",outline="")
        elif fid=="f_jester":
            c.create_oval(x-s//2,y-s,x+s//2,y-s//3,fill=col,outline="#880044",width=1)
            c.create_oval(x-s+2,y-s//3,x+s-2,y+s//2,fill="#ddaa00",outline="#885500",width=1)
            c.create_rectangle(x-s+4,y+s//2,x+s-4,y+s,fill="#cc0066",outline="")
            c.create_polygon(x-s//2,y-s,x-s//2-6,y-s-8,x-s//2+4,y-s-2,fill=col,outline="")
            c.create_oval(x-s//2-8,y-s-10,x-s//2-2,y-s-4,fill="#ffcc00",outline="")

        # ── RARE ──────────────────────────────────────────
        elif fid in ("f_ff_razr","f_ff_slck","f_ff_blze","f_ff_iron"):
            stance={"f_ff_razr":"fire","f_ff_slck":"ice","f_ff_blze":"kick","f_ff_iron":"fist"}[fid]
            # Body
            c.create_oval(x-s//2,y-s,x+s//2,y-s//3,fill=col,outline="#333",width=1)
            c.create_rectangle(x-s//2+2,y-s//3,x+s//2-2,y+s//3,fill=col,outline="#333",width=1)
            if stance=="fire":
                c.create_line(x+s//2-2,y-s//4,x+s+2,y-s,fill=col,width=3)
                for fi_ in range(3):
                    c.create_oval(x+s-2+fi_*3,y-s+2-fi_*3,x+s+4+fi_*3,y-s+8-fi_*3,fill="#ff6600",outline="")
            elif stance=="ice":
                c.create_line(x-s//2+2,y-s//4,x-s-2,y-s+2,fill=col,width=3)
                c.create_polygon(x-s-4,y-s,x-s-8,y-s-4,x-s+2,y-s+4,fill="#88ddff",outline="")
            elif stance=="kick":
                c.create_line(x+4,y+s//3,x+s+2,y-2,fill=col,width=3)
                c.create_oval(x+s-2,y-4,x+s+8,y+4,fill="#ff8800",outline="")
            else:
                c.create_line(x-s//2+2,y-s//4,x-s-2,y,fill=col,width=3)
                c.create_rectangle(x-s-6,y-2,x-s+2,y+4,fill="#888",outline="#555",width=1)
            c.create_line(x-4,y+s//3,x-6,y+s,fill=col,width=3)
            c.create_line(x+4,y+s//3,x+6,y+s,fill=col,width=3)
        elif fid=="f_arena_sm":
            c.create_oval(x-s,y-s//3,x+s,y+s,fill=col,outline="#880000",width=2)
            c.create_oval(x-s+4,y,x+s-4,y+s-2,fill="#1a0000",outline="")
            for ai_ in range(4):
                ax_=x+int((s-3)*math.cos(math.radians(ai_*90))); ay_=y+s//2+int((s//2-2)*math.sin(math.radians(ai_*90)))
                c.create_rectangle(ax_-3,y-s//4,ax_+3,y+s//2,fill="#2a0800",outline=col,width=1)
        elif fid=="f_hotel_sm":
            c.create_rectangle(x-s+4,y-s//3,x+s-4,y+s,fill=col,outline="#8B6914",width=1)
            for fl_ in range(3):
                fy_=y-s//3+fl_*(s//2+2)
                c.create_line(x-s+4,fy_,x+s-4,fy_,fill="#8B6914",width=1)
            c.create_polygon(x-s+4,y-s//3,x,y-s,x+s-4,y-s//3,fill="#8B6914",outline="")
        elif fid=="f_den_sm":
            c.create_rectangle(x-s+4,y-2,x+s-4,y+s,fill=col,outline="#330033",width=1)
            c.create_polygon(x-s+4,y-2,x,y-s+4,x+s-4,y-2,fill="#550055",outline="#330033",width=1)
            c.create_rectangle(x-4,y+s//3,x+4,y+s,fill="#110011",outline="")
            c.create_oval(x-s+6,y,x-s+12,y+6,fill="#aa00aa",outline="")
        elif fid=="f_vip_sm":
            c.create_rectangle(x-s+4,y-2,x+s-4,y+s,fill=col,outline="#2d0d4a",width=1)
            c.create_polygon(x-s+4,y-2,x,y-s+4,x+s-4,y-2,fill="#6a2090",outline="#4a0d6a",width=1)
            c.create_text(x,y+s//2,text="V",fill=GOLD,font=tkfont.Font(size=max(5,s//2),weight="bold"))
        elif fid=="f_dragon":
            c.create_polygon(x,y-s,x-s+2,y+s//2,x,y,x+s-2,y+s//2,fill=col,outline="#550000",width=1)
            c.create_line(x,y-s,x-s,y-s//3,fill=col,width=3); c.create_line(x,y-s,x+s,y-s//3,fill=col,width=3)
            c.create_oval(x-s//3,y-s+2,x+s//3,y-s//2,fill=col,outline="#550000",width=1)
            c.create_oval(x-3,y-s+4,x,y-s+8,fill="#ffaa00",outline="")
        elif fid=="f_mermaid":
            c.create_oval(x-s//2,y-s,x+s//2,y-s//4,fill="#f0c8a0",outline="#c0a080",width=1)
            c.create_oval(x-s+2,y-s//4,x+s-2,y+s//2,fill=col,outline="#1a7a8a",width=1)
            c.create_polygon(x-s+2,y+s//2,x,y+s+4,x+s-2,y+s//2,fill=col,outline="#1a7a8a",width=1)
            c.create_line(x-s//2,y-s,x-s+2,y-s//4,fill="#cc6688",width=2)
        ...
        # ── EPIC ──────────────────────────────────────────
            ...
            for ri_ in range(3):
                r_=s-ri_*4
                c.create_oval(x-r_,y-r_//2,x+r_,y+r_//2,fill="",outline=col,width=max(2,4-ri_))
            c.create_oval(x+s-8,y-s//2-4,x+s+2,y-s//2+6,fill=col,outline="#886600",width=1)
            pts_=[]
            for ci_ in range(5):
                a_=math.radians(ci_*72-90); ai_=math.radians(ci_*72+36-90)
                pts_+=[x+s-2+int(6*math.cos(a_)),y-s//2+1+int(6*math.sin(a_)),
                       x+s-2+int(3*math.cos(ai_)),y-s//2+1+int(3*math.sin(ai_))]
            c.create_polygon(pts_,fill=GOLD,outline="#886600",width=1)
        elif fid=="f_ff_champ":
            c.create_rectangle(x-s+4,y-s//3,x+s-4,y+s//3,fill="#c8a000",outline=GOLD,width=2)
            c.create_rectangle(x-s//4,y-s,x+s//4,y-s//3,fill="#c8a000",outline=GOLD,width=1)
            c.create_oval(x-s//4-4,y-s-4,x-s//4+4,y-s+4,fill=GOLD,outline="")
            c.create_oval(x+s//4-4,y-s-4,x+s//4+4,y-s+4,fill=GOLD,outline="")
            c.create_text(x,y,text="★",fill=GOLD,font=tkfont.Font(size=max(6,s),weight="bold"))
        elif fid=="f_phoenix":
            c.create_polygon(x,y-s,x-s+2,y+s//2,x,y,x+s-2,y+s//2,fill=col,outline="#cc4400",width=1)
            for fi2_ in range(4):
                fa_=math.radians(fi2_*45-90)
                c.create_line(x,y-s//2,x+int((s+4)*math.cos(fa_)),y-s//2+int((s+4)*math.sin(fa_)),fill="#ff6600",width=2)
                c.create_oval(x+int(s*math.cos(fa_))-3,y-s//2+int(s*math.sin(fa_))-3,
                              x+int(s*math.cos(fa_))+3,y-s//2+int(s*math.sin(fa_))+3,fill="#ffcc00",outline="")
        elif fid=="f_kraken":
            c.create_oval(x-s//2,y-s//2,x+s//2,y+s//2,fill=col,outline="#112244",width=2)
            for ti_ in range(6):
                ta_=math.radians(ti_*60); tx_=x+int(s*math.cos(ta_)); ty_=y+int(s*math.sin(ta_))
                c.create_line(x,y,tx_,ty_,fill=col,width=max(3,s//4))
                c.create_oval(tx_-4,ty_-4,tx_+4,ty_+4,fill="#224466",outline="")
        elif fid=="f_golem":
            c.create_oval(x-s//2,y-s,x+s//2,y-s//3,fill=col,outline="#554433",width=2)
            c.create_rectangle(x-s+2,y-s//3,x+s-2,y+s//3,fill=col,outline="#554433",width=2)
            c.create_rectangle(x-s//4,y+s//3,x+s//4,y+s,fill=col,outline="#554433",width=1)
            for gx_ in [x-s,x+s-6]:
                c.create_rectangle(gx_,y-s//4,gx_+8,y+s//4,fill="#778866",outline="#554433",width=1)

        # ── LEGENDARY ─────────────────────────────────────
        elif fid=="f_casino_big":
            c.create_rectangle(x-s,y-s//4,x+s,y+s,fill=col,outline="#880000",width=2)
            for wx_ in [x-s+4,x-2,x+s-10]:
                c.create_rectangle(wx_,y-s//4+4,wx_+6,y+s-4,fill="#1a0028",outline="#cc00cc",width=1)
            c.create_polygon(x-s,y-s//4,x,y-s,x+s,y-s//4,fill="#aa1100",outline="#880000",width=1)
            c.create_text(x,y+s-6,text="★",fill="#ffcc00",font=tkfont.Font(size=max(5,s//2)))
        elif fid=="f_colossus":
            c.create_oval(x-s//2,y-s,x+s//2,y-s//3,fill=col,outline="#665544",width=2)
            c.create_rectangle(x-s+2,y-s//3,x+s-2,y+s//4,fill=col,outline="#665544",width=2)
            c.create_rectangle(x-s//2,y+s//4,x-s//4,y+s,fill=col,outline="#665544",width=1)
            c.create_rectangle(x+s//4,y+s//4,x+s//2,y+s,fill=col,outline="#665544",width=1)
            c.create_line(x-s+2,y-s//6,x-s-6,y+s//4,fill=col,width=max(3,s//4))
            c.create_line(x+s-2,y-s//6,x+s+6,y+s//4,fill=col,width=max(3,s//4))
        elif fid=="f_unicorn":
            c.create_oval(x-s+4,y-s+8,x+s-2,y+6,fill=col,outline="#cc88ff",width=1)
            c.create_oval(x-s+2,y-s-2,x-s+12,y-s+10,fill=col,outline="#cc88ff",width=1)
            c.create_line(x-s+8,y-s+8,x-s+4,y-s+2,fill=col,width=3)
            for li_,lx_ in enumerate([x-s+6,x-s+10,x+s-12,x+s-8]):
                c.create_line(lx_,y+6,lx_+(1 if li_%2 else -1)*2,y+s,fill=col,width=3)
            c.create_line(x+s-4,y-s+10,x+s+4,y-s,fill="#ff88ff",width=3)
            # Spiral horn
            c.create_line(x-s+6,y-s+2,x-s,y-s-8,fill=GOLD,width=2)
            c.create_line(x-s+4,y-s-2,x-s+2,y-s-8,fill="#ffccff",width=1)
        elif fid=="f_sphinx":
            c.create_oval(x-s+2,y-s//3,x+s-2,y+s,fill=col,outline="#8a6820",width=1)
            c.create_oval(x-s//2,y-s,x+s//2,y-s//4,fill=col,outline="#8a6820",width=1)
            c.create_rectangle(x-s+2,y-s//3-4,x+s-2,y-s//3+4,fill="#cc9930",outline="")
            c.create_oval(x-4,y-s//2,x+4,y-s//2+8,fill="#ffcc44",outline="")
        elif fid=="f_valkyrie":
            c.create_oval(x-s//2,y-s,x+s//2,y-s//3,fill="#f0c8a0",outline="#aaa",width=1)
            c.create_rectangle(x-s//2+2,y-s//3,x+s//2-2,y+s//4,fill=col,outline=rc,width=1)
            c.create_line(x-s//2+2,y-s//3+4,x-s-4,y,fill=col,width=3)
            c.create_line(x+s//2-2,y-s//3+4,x+s+4,y,fill=col,width=3)
            c.create_rectangle(x-s//4,y+s//4,x+s//4,y+s,fill=col,outline=rc,width=1)
            c.create_line(x+s//2-2,y-s,x+s+2,y-s-s,fill=GOLD,width=2)  # spear
            c.create_polygon(x+s+2,y-s-s,x+s-2,y-s-s+6,x+s+6,y-s-s+4,fill=GOLD,outline="")  # tip
        elif fid in ("f_serpent","f_leviathan"):
            rings=4 if fid=="f_serpent" else 5
            for ri_ in range(rings):
                r_=s-ri_*(s//rings)
                c.create_oval(x-r_,y-r_//2,x+r_,y+r_//2,fill="",outline=col,width=max(2,4-ri_))
            sz_=5 if fid=="f_leviathan" else 4
            c.create_oval(x+s-sz_*2,y-s//3-sz_,x+s+sz_,y-s//3+sz_,fill=col,outline="#001a22",width=1)
            c.create_oval(x+s-sz_,y-s//3-2,x+s+sz_-2,y-s//3+2,fill="#cc0000",outline="")
        elif fid=="f_titan":
            # Massive crouching shape
            c.create_oval(x-s//2,y-s,x+s//2,y-s//3,fill=col,outline="#444433",width=2)
            c.create_rectangle(x-s,y-s//3,x+s,y+s//3,fill=col,outline="#444433",width=2)
            c.create_rectangle(x-s,y+s//3,x-s//3,y+s,fill=col,outline="#444433",width=1)
            c.create_rectangle(x+s//3,y+s//3,x+s,y+s,fill=col,outline="#444433",width=1)

        # ── MYTHIC ────────────────────────────────────────
        elif fid=="f_myth_rg":
            # All-seeing deity — radiant star shape
            for mi_ in range(8):
                ma_=math.radians(mi_*45); mb_=math.radians(mi_*45+22)
                c.create_line(x,y,x+int(s*math.cos(ma_)),y+int(s*math.sin(ma_)),fill=col,width=2)
                c.create_line(x,y,x+int((s//2)*math.cos(mb_)),y+int((s//2)*math.sin(mb_)),fill=rc,width=1)
            c.create_oval(x-s//3,y-s//3,x+s//3,y+s//3,fill=col,outline=rc,width=2)
            c.create_text(x,y,text="R",fill=rc,font=tkfont.Font(size=max(5,s//2),weight="bold"))
        elif fid=="f_myth_time":
            c.create_oval(x-s,y-s,x+s,y+s,fill=col,outline=rc,width=2)
            c.create_line(x,y,x,y-s+4,fill="#888",width=2)  # 12
            c.create_line(x,y,x+s-4,y,fill="#666",width=1)  # 3
            for hi_ in range(12):
                ha_=math.radians(hi_*30-90); hx_=x+int((s-3)*math.cos(ha_)); hy_=y+int((s-3)*math.sin(ha_))
                c.create_oval(hx_-1,hy_-1,hx_+1,hy_+1,fill=rc,outline="")
        elif fid=="f_myth_void":
            # Ouroboros — snake eating tail
            c.create_oval(x-s+2,y-s+2,x+s-2,y+s-2,fill="",outline=col,width=max(4,s//3))
            head_x=x+s-4; head_y=y
            c.create_oval(head_x-4,head_y-4,head_x+4,head_y+4,fill=col,outline=rc,width=1)
            c.create_oval(head_x-1,head_y-1,head_x+1,head_y+1,fill="#880000",outline="")
        elif fid=="f_myth_sun":
            for ray_ in range(12):
                ra_=math.radians(ray_*30)
                c.create_line(x+int(s//2*math.cos(ra_)),y+int(s//2*math.sin(ra_)),
                              x+int(s*math.cos(ra_)),y+int(s*math.sin(ra_)),fill=col,width=2)
            c.create_oval(x-s//2,y-s//2,x+s//2,y+s//2,fill=col,outline="#ff8800",width=2)
        elif fid=="f_myth_chs":
            # Chaos swirl — overlapping rarity colour arcs
            for ci_,rcol_ in enumerate(self.RARITY_COLS.values()):
                ca_=ci_*60; c.create_arc(x-s+3,y-s+3,x+s-3,y+s-3,start=ca_,extent=55,
                                          outline=rcol_,style="arc",width=max(2,s//5))
            c.create_oval(x-s//4,y-s//4,x+s//4,y+s//4,fill="#ffffff",outline=rc,width=1)

        else:
            # Fallback generic humanoid
            c.create_oval(x-s//2,y-s,x+s//2,y-s//3,fill=col,outline="#333",width=1)
            c.create_rectangle(x-s//2+2,y-s//3,x+s//2-2,y+s//3,fill=col,outline="#333",width=1)
            c.create_line(x-s//2+2,y-s//4,x-s,y+s//6,fill=col,width=3)
            c.create_line(x+s//2-2,y-s//4,x+s,y+s//6,fill=col,width=3)
            c.create_line(x-4,y+s//3,x-6,y+s,fill=col,width=3)
            c.create_line(x+4,y+s//3,x+6,y+s,fill=col,width=3)
    ...
    def _figurine_room_screen(self,room_key,table_idx=0,scroll_y=None):
        """Per-table figurine display manager — 3 independent tables per room."""
        self._clear_overlay(); c=self.canvas
        if not hasattr(self,"figurine_display_rooms"): self.figurine_display_rooms={}
        tbl_key=f"{room_key}_t{table_idx}"
        placed=list(self.figurine_display_rooms.get(tbl_key,[]))
        self.figurine_display_rooms[tbl_key]=placed

        is_suite=room_key.startswith("room_2")
        max_slots=6 if is_suite else 4   # per-table limit (not huge — 3 tables per room)
        accent="#00aaaa" if is_suite else GOLD
        col1="#061014" if is_suite else "#0a0d08"
        col2="#082018" if is_suite else "#0d1a10"
        tbl_names=["A","B","C"]
        self._draw_room_bg(
            f"TABLE {tbl_names[table_idx]}  —  {'SUITE' if is_suite else 'ROOM'} DISPLAY",col1,col2)

        # Tab strip to switch between tables
        for ti,tname in enumerate(tbl_names):
            tx=W//2-120+ti*120
            active=ti==table_idx
            round_rect(c,tx-50,58,tx+50,84,r=8,
                       fill="#1a2a1a" if active else "#0d1408",
                       outline=accent if active else "#333",width=2 if active else 1)
            tbl_placed_count=len(self.figurine_display_rooms.get(f"{room_key}_t{ti}",[]))
            c.create_text(tx,71,text=f"Table {tname}  {tbl_placed_count}/{max_slots}",
                          fill=accent if active else "#666",
                          font=tkfont.Font(family="Courier New",size=8,weight="bold" if active else "normal"))
            if not active:
                tag=f"tabtab_{ti}"
                c.create_rectangle(tx-50,58,tx+50,84,fill="",outline="",tags=tag)
                c.tag_bind(tag,"<Button-1>",
                           lambda e,t=ti: self._figurine_room_screen(room_key,table_idx=t))

        # ── On-display shelf: big separated slots ──
        SLOT_W=140; SLOT_H=140; GAP=24
        total_row_w=max_slots*(SLOT_W+GAP)-GAP
        sx0=(W-total_row_w)//2
        c.create_text(W//2,102,text="─── On Display  (click ✕ to remove) ───",
                      fill="#666",font=tkfont.Font(family="Courier New",size=7))
        for i in range(max_slots):
            gx=sx0+i*(SLOT_W+GAP)+SLOT_W//2; gy=115
            filled=i<len(placed)
            round_rect(c,gx-SLOT_W//2,gy,gx+SLOT_W//2,gy+SLOT_H,r=10,
                       fill="#0c1a0c" if filled else col1,
                       outline=accent if filled else "#2a2a2a",width=2 if filled else 1)
            if filled:
                fid=placed[i]
                self._draw_figurine(c,fid,gx,gy+56,size=52)   # big figurine
                fn=(self.FIGURINE_BY_ID[fid][1] if fid in self.FIGURINE_BY_ID else fid)
                rar=(self.FIGURINE_BY_ID[fid][2] if fid in self.FIGURINE_BY_ID else "")
                rc=self.RARITY_COLS.get(rar,"#aaa")
                c.create_text(gx,gy+108,text=fn[:13],fill=rc,
                              font=tkfont.Font(family="Courier New",size=7,weight="bold"))
                c.create_text(gx,gy+120,text=rar.upper(),fill=rc,
                              font=tkfont.Font(family="Courier New",size=6))
                def _make_remove(idx2,pl=placed,tk=tbl_key,sy=scroll_y):
                    def do():
                        pl.pop(idx2)
                        self.figurine_display_rooms[tk]=pl
                        self._figurine_room_screen(room_key,table_idx,sy)
                    return do
                self._make_btn(gx,gy+134,"✕",_make_remove(i),col="#2a0010",fg="#ff6666",w=26)
            else:
                c.create_text(gx,gy+SLOT_H//2,text="empty",fill="#2a2a2a",
                              font=tkfont.Font(family="Courier New",size=8))

        # ── Collection picker (scrollable) ──
        SEP_Y=268
        c.create_line(30,SEP_Y,W-30,SEP_Y,fill="#2a2a2a",width=1)
        c.create_text(W//2,SEP_Y+10,text="─── Your collection  (click to place) ───",
                      fill="#666",font=tkfont.Font(family="Courier New",size=7))

        coll=getattr(self,"figurine_collection",[])
        placed_set=set(placed)
        avail=[fid for fid in dict.fromkeys(coll) if fid not in placed_set]

        VIEW_Y=SEP_Y+24; VIEW_H=H-VIEW_Y-44
        CCOLS=6; CW2=118; CH2=110; CGAP=10
        scroll_key=f"{tbl_key}_scroll"
        if scroll_y is None:
            scroll_y=getattr(self,"_fig_room_scroll",{}).get(scroll_key,0)
        rows=max(1,math.ceil(len(avail)/CCOLS))
        content_h=rows*(CH2+CGAP)+CGAP
        max_scroll=max(0,content_h-VIEW_H)
        scroll_y=max(0,min(scroll_y,max_scroll))
        if not hasattr(self,"_fig_room_scroll"): self._fig_room_scroll={}
        self._fig_room_scroll[scroll_key]=scroll_y

        c.create_rectangle(0,VIEW_Y,W,VIEW_Y+VIEW_H,fill=col1,outline="")

        if not avail:
            c.create_text(W//2,VIEW_Y+VIEW_H//2,
                          text="No figurines available to place.\nVisit the Gumball Emporium!",
                          fill="#445544",font=tkfont.Font(family="Courier New",size=9),
                          justify="center")
        row_start_x=(W-CCOLS*(CW2+CGAP)+CGAP)//2
        for idx,fid in enumerate(avail):
            col_=idx%CCOLS; row_=idx//CCOLS
            cx2=row_start_x+col_*(CW2+CGAP)+CW2//2
            cy2=VIEW_Y+CGAP+row_*(CH2+CGAP)+CH2//2-scroll_y
            if cy2+CH2//2<VIEW_Y or cy2-CH2//2>VIEW_Y+VIEW_H: continue
            if fid not in self.FIGURINE_BY_ID: continue
            _,name,rarity,_,_=self.FIGURINE_BY_ID[fid]
            rc2=self.RARITY_COLS.get(rarity,"#aaa")
            round_rect(c,cx2-CW2//2,cy2-CH2//2,cx2+CW2//2,cy2+CH2//2,r=8,
                       fill="#100c18",outline=rc2,width=1)
            self._draw_figurine(c,fid,cx2,cy2-14,size=44)
            c.create_text(cx2,cy2+34,text=name[:13],fill=rc2,
                          font=tkfont.Font(family="Courier New",size=7,weight="bold"),width=CW2-6)
            c.create_text(cx2,cy2+46,text=rarity.upper(),fill=rc2,
                          font=tkfont.Font(family="Courier New",size=6))
            btn_tag=f"frsp_{idx}"
            round_rect(c,cx2-28,cy2+52,cx2+28,cy2+64,r=4,
                       fill="#0a2010",outline=accent,width=1,tags=btn_tag)
            c.create_text(cx2,cy2+58,text="▶ Place",fill=accent,
                          font=tkfont.Font(family="Courier New",size=6,weight="bold"),tags=btn_tag)
            def _make_place(fid2=fid,pl=placed,tk=tbl_key,sy=scroll_y):
                def do():
                    if len(pl)>=max_slots:
                        self._msg("Table full!",RED_C); return
                    pl.append(fid2)
                    ...
                    self._figurine_room_screen(room_key,table_idx,sy)
                return do
            c.tag_bind(btn_tag,"<Button-1>",lambda e,fn=_make_place():fn())

        # ── Clip overflow below collection view ──
        c.create_rectangle(0,VIEW_Y+VIEW_H,W,H,fill=col1,outline="")

        # ── Scroll arrows (always draw; grey when at limit) ──
        up_col=accent if scroll_y>0 else "#333"
        dn_col=accent if scroll_y<max_scroll else "#333"
        up_id=c.create_text(W-18,VIEW_Y+16,text="▲",fill=up_col,font=tkfont.Font(size=11))
        dn_id=c.create_text(W-18,VIEW_Y+VIEW_H-16,text="▼",fill=dn_col,font=tkfont.Font(size=11))
        c.tag_bind(up_id,"<Button-1>",
                   lambda e:self._figurine_room_screen(room_key,table_idx,scroll_y-90))
        c.tag_bind(dn_id,"<Button-1>",
                   lambda e:self._figurine_room_screen(room_key,table_idx,scroll_y+90))

        # ── Mouse wheel — always active ──
        def _whl(e):
            dy=-1 if (e.delta>0 or e.num==4) else 1
            self._figurine_room_screen(room_key,table_idx,scroll_y+dy*70)
        c.bind("<MouseWheel>",_whl); c.bind("<Button-4>",_whl); c.bind("<Button-5>",_whl)

        # ── Back button on top of clip rect ──
        self._make_btn(W//2,H-26,"← Back",self._back_to_interior,col="#1a1828",fg=CREAM,w=120)
    ...
    def _gumball_screen(self,_=None):
        ...
        fig_drawn=[False]; rolling=[False]; fig_items=[]
    ...
    def _make_hotel_rooms(self):
        ...
        def room_decor_f2(c):
            ...
            # Three figurine display shelves on right wall
            shelf_xs=[W//2+220,W//2+220,W//2+220]
            shelf_ys=[290, 390, 490]
            shelf_labels=["A","B","C"]
            for si,(dsx,dsy,slbl) in enumerate(zip(shelf_xs,shelf_ys,shelf_labels)):
                c.create_rectangle(dsx-70,dsy,dsx+70,dsy+8,fill="#5a3818",outline=GOLD,width=1)
                c.create_rectangle(dsx-66,dsy+8,dsx+66,dsy+11,fill="#3a2210",outline="")
                for lg in [-50,50]:
                    c.create_rectangle(dsx+lg-4,dsy+11,dsx+lg+4,dsy+32,fill="#4a2a10",outline="#3a1a08")
                for sli in range(4):
                    sx_=dsx-48+sli*32
                    c.create_oval(sx_-5,dsy-7,sx_+5,dsy+1,fill="#2a1808",outline="#8B6914",width=1)
                c.create_text(dsx,dsy-14,text=f"Table {slbl}",fill=GOLD,
                              font=tkfont.Font(family="Courier New",size=6,weight="bold"))
        ...
        def room_decor_f3(c):
            ...
            # Three figurine display shelves on right wall (teal luxury version)
            shelf_xs3=[W//2+220,W//2+220,W//2+220]
            shelf_ys3=[290, 390, 490]
            shelf_labels3=["A","B","C"]
            for si,(dsx3,dsy3,slbl3) in enumerate(zip(shelf_xs3,shelf_ys3,shelf_labels3)):
                c.create_rectangle(dsx3-76,dsy3,dsx3+76,dsy3+10,fill="#0d4a38",outline="#00aaaa",width=2)
                c.create_rectangle(dsx3-72,dsy3+10,dsx3+72,dsy3+14,fill="#0a3828",outline="")
                for lg3 in [-56,56]:
                    c.create_rectangle(dsx3+lg3-4,dsy3+14,dsx3+lg3+4,dsy3+38,fill="#082a20",outline="#005a44")
                for sli3 in range(6):
                    sx3_=dsx3-60+sli3*24
                    c.create_oval(sx3_-5,dsy3-7,sx3_+5,dsy3+1,fill="#0a2820",outline="#00aaaa",width=1)
                c.create_text(dsx3,dsy3-14,text=f"Table {slbl3}",fill="#00aaaa",
                              font=tkfont.Font(family="Courier New",size=6,weight="bold"))
        ...
        # Three display table positions per room (right side, spread vertically)
        F2_TABLES=lambda rk:[
            {"id":f"ft_{rk}_0","x":W//2+200,"y":310,"name":"Display Table A",
             "line":"Display Table A\n[C] to manage","game":f"fig_room_{rk}_0"},
            {"id":f"ft_{rk}_1","x":W//2+200,"y":420,"name":"Display Table B",
             "line":"Display Table B\n[C] to manage","game":f"fig_room_{rk}_1"},
            {"id":f"ft_{rk}_2","x":W//2+200,"y":530,"name":"Display Table C",
             "line":"Display Table C\n[C] to manage","game":f"fig_room_{rk}_2"},
        ]
        F3_TABLES=lambda rk:[
            {"id":f"ft_{rk}_0","x":W//2+200,"y":310,"name":"Display Table A",
             "line":"Display Table A\n[C] to manage","game":f"fig_room_{rk}_0"},
        ...
        def show_dialogue():
            ...
            if is_collection:
                ...
                def redraw_panel():
                    ...
                    draw_fn(tx,ty)  # redraw NPC cleanly
                ...
                if self.money>0 and self.money>=take:
                    ...
                    def cant_pay():
                        ...
                        def force_work():
                            # Minimal cleanup — no loop restart, go straight to work screen
                            c.delete(NPC,"boss_dlg")
                            for w in list(self._overlay_widgets):
                                try: w.destroy()
                                except: pass
    ...
    def _work_off_debt_screen(self):
        self.screen="game"; self._esc_blocked=True
